# Notebook Technique Audit — `07.tsr_colab_production_lite_demo.ipynb`

> **Mục tiêu file này:** trả lời trực tiếp ba câu hỏi cho notebook `07.tsr_colab_production_lite_demo.ipynb`:  
...
> **Phạm vi:** notebook hiện tại trên branch `duong_split_research`, có đối chiếu trực tiếp với cùng path ở ref `origin/vinhnpq/pha-hoai` để tách phần bổ sung downstream; không đánh đồng mọi khác biệt đó là nội dung của notebook hiện tại.
...
- Có một điểm lệch giữa phần mô tả và code: markdown cell đầu notebook nhắc state `SUPPRESSED`, nhưng code thực tế **không có** state riêng này.
...
### 5.1 Có mismatch giữa markdown và code về `SUPPRESSED`

Markdown đầu notebook nói lifecycle có:
...
Nhưng code thực tế ở cell `28` chỉ thể hiện rõ:
...
## 6. Nội dung này đang nằm ở đâu trong các `.md` hiện có

Nếu chỉ cần tài liệu gần nhất với notebook mà không cần audit riêng, các phần liên quan hiện nằm rải ở đây:

| File | Nội dung đã có |
|---|---|
| [02.colab_production_lite_demo.md](02.colab_production_lite_demo.md) | Tóm tắt notebook làm được gì ở mức entrypoint |
| [05.colab_production_lite_demo_full.md](05.colab_production_lite_demo_full.md) | Mô tả đầy đủ pipeline production-lite, KPI proxy, GT eval, map-fusion stub |
| [04.baseline_repo_analysis_full.md](04.baseline_repo_analysis_full.md) | Phân biệt baseline `tsr_demo.py`/`best.pt` với các block notebook đã bổ sung |
| [../2.knowledge_base/02.state_manager_and_sign_lifecycle.md](../2.knowledge_base/02.state_manager_and_sign_lifecycle.md) | Giải thích lifecycle, stage pipeline và đã nêu rõ notebook chưa có state `SUPPRESSED` riêng |
| [../2.knowledge_base/06.hmi_diagnostics_and_vehicle_interface.md](../2.knowledge_base/06.hmi_diagnostics_and_vehicle_interface.md) | Giải thích `warning level`, HMI priority ladder và phân biệt output layer |

Tuy vậy, trước khi thêm file này thì repo **chưa có** một tài liệu nào tách bạch rõ ba câu hỏi:

- phần nào là model có sẵn;
- phần nào là notebook logic bạn tự code;
- và branch nào thực sự đã hoặc chưa được tích hợp.

Đó là lý do file audit này được thêm riêng.
