...
> **Folder index:** [00.index.md](00.index.md) — Lấy: cấu trúc implementation layer và thứ tự đọc. Dùng ở: điều hướng người đọc từ full baseline analysis sang overview/notebook/benchmark.
>
> **File này trả lời:** baseline repo hiện tại làm được gì, thiếu gì, và gap đó biểu hiện ra sao trên runtime thực tế.
>
> **Ngoài phạm vi:** giải thích đầy đủ production framework, V-model, safety/SOTIF hay kiến trúc tổng thể; phần đó nằm ở [Unified Production Reference](../2.knowledge_base/12.unified_production_reference.md) — Lấy: kiến trúc production và roadmap hệ thống. Dùng ở: điều hướng khi gap baseline cần giải thích ở level feature/system.
...
- Khi cần lý do hệ thống-level "vì sao detector chưa phải feature", quay lại [Unified Production Reference](../2.knowledge_base/12.unified_production_reference.md) — deep dive production về feature TSR đầy đủ.
- Khi cần theory detector, quay lại [Detector Architecture Deep Reference](../2.knowledge_base/13.detector_architecture_deep_reference.md) — phân tích kiến trúc detector, NMS, multi-scale và edge deployment.
...
- repo hiện đã có thêm notebook sidecar [notebooks/07.tsr_colab_production_lite_demo.ipynb](notebooks/07.tsr_colab_production_lite_demo.ipynb) mô phỏng `quality gate`, `ODD gate`, `IoU tracking`, `track lifecycle`, `warning level`, `events.jsonl`, KPI proxy và optional GT eval;
...
> **Ghi chú nguồn:** bảng này là số đo benchmark local lịch sử của baseline `tsr_demo.py`, không phải cấu hình mặc định hiện tại của notebook `07`. Notebook production-lite hiện dùng profile `high/medium` với `skip=0`, `low` với `skip=1`; video replay hiện có trong repo là `videos/14212806_3840_2160_60fps.mp4` (`3840x2160`, `1187` frame, `59.94 fps`). Xem thêm [Colab Production-Lite Demo](02.colab_production_lite_demo.md) và [Notebook Analyze](10.notebook_analyze.md) để tránh nhầm giữa video FPS, scheduled inference rate và measured processing FPS.
