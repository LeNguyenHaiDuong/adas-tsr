...
# CLAHE trong Pipeline TSR: Từ BGR → XYZ → LAB → Contrast Enhancement

## 1. Mục tiêu

Trong bài toán Traffic Sign Recognition (TSR), chất lượng ảnh đầu vào ảnh hưởng trực tiếp đến khả năng phát hiện biển báo của mô hình YOLO.

Các vấn đề thường gặp:

- Ảnh thiếu sáng
- Ảnh có độ tương phản thấp
- Biển báo nằm trong bóng cây
- Biển báo bị ngược sáng
- Điều kiện thời tiết làm giảm khả năng quan sát

Để cải thiện khả năng nhận diện, pipeline sử dụng:

```python
frame = apply_clahe(frame)
```

CLAHE (Contrast Limited Adaptive Histogram Equalization) được áp dụng trên kênh độ sáng của ảnh nhằm tăng tương phản cục bộ nhưng vẫn giữ nguyên màu sắc của biển báo.

---

# 2. Tại sao không áp dụng CLAHE trực tiếp trên BGR?

Một pixel trong ảnh BGR được biểu diễn bởi:
...
Đây là một pixel màu đỏ.

Nếu thực hiện Histogram Equalization trực tiếp trên từng kênh:
...
thì màu sắc có thể bị biến dạng:
...
Trong TSR, màu sắc là đặc trưng quan trọng:

- Biển cấm: đỏ
- Biển chỉ dẫn: xanh
- Biển cảnh báo: vàng

Do đó không nên thay đổi trực tiếp các kênh màu.

---

# 3. Chuyển đổi từ BGR sang LAB

Để xử lý riêng phần độ sáng, ảnh được chuyển sang không gian màu LAB.
...
XYZ là không gian màu chuẩn hóa bởi CIE nhằm mô phỏng cách mắt người cảm nhận ánh sáng.

Trong XYZ:

```text
X = thành phần màu thứ nhất
Y = độ sáng cảm nhận (Luminance)
Z = thành phần màu thứ ba
```
...
LAB được thiết kế để tách riêng:
...
A và B không phải là màu đơn lẻ.

Chúng là các trục màu đối nghịch.
...
# 6. Tại sao CLAHE chỉ áp dụng trên L?
...
```text
Không làm thay đổi màu sắc biển báo
```
...
A và B được giữ nguyên hoàn toàn.
...
# 8. Step 1: Tách kênh L

Sau khi chuyển sang LAB:
...
Các giá trị chỉ nằm trong khoảng:
...
CLAHE đếm số lượng pixel của từng mức sáng.
...
Đây chính là histogram.

---

# 11. Step 4: Phát hiện độ tương phản thấp

CLAHE nhận thấy:

```text
Pixel chỉ nằm trong khoảng 92 → 100
```

trong khi dải sáng lý thuyết là:

```text
0 → 255
```

=> Dynamic range quá hẹp.

---

# 12. Step 5: Histogram Equalization

CLAHE bắt đầu kéo giãn dải sáng.
...
là xác suất xuất hiện của mức sáng:
...
Nếu histogram có đỉnh quá lớn:

```text
95 : ██████████████████████████
```

thì CLAHE cắt ngưỡng:
...
# 14. Step 7: Phân phối lại phần dư

Các pixel bị cắt bỏ không bị mất.

Chúng được phân phối lại cho các mức sáng khác.
...
được phân bổ cho các bin khác.

---

# 15. Step 8: Lặp lại trên toàn bộ Tile
...
```text
Vùng tối được tăng tương phản

Vùng sáng vẫn được giữ ổn định
```

---

# 16. Step 9: Bilinear Interpolation
...
để làm mượt biên giữa các tile.

---

# 17. Step 10: Tạo L mới
...
# 18. Step 11: Ghép lại với A và B
...
# 19. Step 12: Chuyển LAB về BGR

Cuối cùng:

```python
cv2.cvtColor(limg, cv2.COLOR_LAB2BGR)
```
...
# 20. Lợi ích đối với TSR

CLAHE giúp:

- Tăng tương phản cục bộ
- Làm nổi bật viền biển báo
- Cải thiện khả năng nhận diện trong vùng tối
- Giảm ảnh hưởng của bóng râm
- Giữ nguyên màu sắc biển báo

Đây là lý do CLAHE thường được sử dụng như một bước tiền xử lý trong các hệ thống nhận diện biển báo giao thông và ADAS.
