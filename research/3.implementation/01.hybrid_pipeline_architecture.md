...
## 4.1. Động Cơ Hợp Nhất Quyết Định Hybrid (Bayesian Confidence Fusion Engine)
...
| $0.20 \le \text{conf} < 0.50$ | Phát hiện đúng hình tròn + màu đỏ | **Nâng điểm Fused** | Kích hoạt cờ `RESCUED_BY_CV`, chuyển sang `Candidate` có trọng số cao. |
| $0.20 \le \text{conf} < 0.50$ | CV không tìm thấy hình học tương ứng | **Từ chối (Suppress)** | Coi là nhiễu nền hoặc bóng cây; không kích hoạt cảnh báo. |
| $< 0.20$ | CV phát hiện đường tròn rất rõ | **Chỉ ghi Log (Debug)** | Đánh dấu `POTENTIAL_FALSE_ALARM`; cấm xuất lên HMI để tránh báo giả. |

---

### 4.2. Ví Dụ Số Học Thực Nghiệm: Giải Cứu Biển Báo P.127 Bị Bỏ Sót

**Tình huống thực nghiệm:**
Xe chạy qua cầu Nhật Tân (Hà Nội) lúc 17h30 chiều mưa phùn mù mịt. Phía trước có biển P.127 ($60\text{ km/h}$).
1. **Kết quả từ YOLO11s**:
   - Model trả về detection: Bounding Box $b = [x=450, y=120, w=42, h=40]$, Class = `P.127-60`.
   - Độ tin cậy thấp: $\text{conf}_{yolo} = 0.22$ (thấp hơn ngưỡng lọc mặc định `threshold_high = 0.35`). Nếu chạy baseline thông thường, detection này sẽ bị **vứt bỏ ngay lập tức (False Negative)**.
2. **Kích hoạt nhánh Classical CV**:
   - Cắt ROI kích thước $60 \times 60$ bao quanh tọa độ $[450, 120]$.
   - Áp dụng CLAHE làm nổi bật dải màu đỏ viền ngoài.
   - Phân đoạn HSV thu được $1,320\text{ pixels}$ màu đỏ.
   - Tìm contour lớn nhất: Diện tích $A = 1,260\text{ px}^2$, chu vi $P = 129\text{ px}$.
   - Tính độ tròn:
     $$\mathcal{C} = \frac{4 \pi \times 1,260}{129^2} = \frac{15,833.6}{16,641} \approx 0.9515 \quad (\text{Rất gần 1.0!})$$
   - Biến đổi Hough Circle tìm thấy tâm đường tròn tại $(x_c=471, y_c=140)$, bán kính $r = 20\text{ px}$.
   - Tâm hộp YOLO có tọa độ $(x_c=471, y_c=140)$ $\rightarrow$ Sai lệch tâm $\Delta = 0.0\text{ pixel}$!
   - Điểm đánh giá CV: $\text{score}_{cv} = 0.92$.
3. **Tính toán điểm hợp nhất (Fused Confidence)**:
   $$P_{fused} = 0.60 \times 0.22 + 0.40 \times 0.92 + 0.15 = 0.132 + 0.368 + 0.15 = 0.650$$
4. **Kết luận**:
   Điểm tin cậy được nâng từ **$0.22$ lên $0.65$** ($\ge 0.50$). Biển báo được giải cứu thành công, chuyển sang `StateManager` để đưa lên taplo cảnh báo cho tài xế trước $2.5\text{ giây}$!
...
flowchart LR
    ...
    T1["Luồng 1: OpenCV Frame Reader (Camera/Video)"]:::layerSensing --> Q1["Frame FIFO Queue (Giới hạn depth=2)"]:::nodeQueue
    Q1 --> T2["Luồng 2: YOLO Inference Worker (GPU/NPU)"]:::layerPerception
    T2 --> Q2["Detection FIFO Queue (depth=2)"]:::nodeQueue
    Q2 --> T3["Luồng 3: State Manager FSM"]:::layerState
