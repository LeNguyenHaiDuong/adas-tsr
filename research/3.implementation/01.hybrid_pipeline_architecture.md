...
flowchart LR
    ...
    subgraph L1 ["LAYER 1: SENSING"]
        A["Video / Webcam Source"]:::layerSensing --> B["Frame Capture"]:::layerSensing
    end

    subgraph L2 ["LAYER 2: CONDITIONING & QUALITY"]
        C["Resize Preprocess (max_width)"]:::layerQuality
    end

    subgraph L3 ["LAYER 3: HYBRID PERCEPTION"]
        D["YOLO11s Detection"]:::layerPerception
        E["Optional HSV + Contour Heuristic"]:::layerPerception
        F["Merge Candidates + NMS"]:::layerPerception
    end

    subgraph L5 ["LAYER 5: STATE LIFECYCLE"]
        G["Hold-based Temporal Lite / FSM"]:::layerState
    end

    subgraph L6 ["LAYER 6: OUTPUT & HMI"]
        H["Overlay BBox & Warning Text"]:::layerOutput --> I["Output Video / Display"]:::layerOutput
    end
...
## 3. Chi Tiết Nhánh Deep Learning — Kiến Trúc Mạng Ultralytics YOLO11s

Nhánh nhận diện chính trong repo `adas-tsr` sử dụng mô hình **Ultralytics YOLO11s** (checkpoint `models/best.pt`, huấn luyện trên 82 lớp biển báo Việt Nam). Kiến trúc mạng được thiết kế gồm ba phần:

```mermaid
flowchart TD
    subgraph Backbone["1. Backbone Trích Xuất Đặc Trưng"]
        B1["Conv 3x3 (Stride 2)"] --> B2["C3k2 Block 1"]
        B2 --> B3["Conv 3x3 (Stride 2)"]
        B3 --> B4["C3k2 Block 2"]
        B4 --> B5["Conv 3x3 (Stride 2)"]
        B5 --> B6["C3k2 Block 3"]
        B6 --> B7["SPPF (Spatial Pyramid Pooling)"]
        B7 --> B8["C2PSA (Pointwise Spatial Attention)"]
    end

    subgraph Neck["2. Neck Đa Quy Mô (PAN-FPN)"]
        B8 --> N1["Top-down Upsampling + Concat"]
        N1 --> N2["C3k2 Fusion Blocks"]
        N2 --> N3["Bottom-up Downsampling + Concat"]
    end

    subgraph Head["3. Decoupled Detection Head (Anchor-free)"]
        N3 --> H1["Nhánh Hồi Quy Tọa Độ Box (CIoU + DFL)"]
        N3 --> H2["Nhánh Phân Loại Xác Suất Class (BCE Loss)"]
    end
```

### 3.1. Các Khối Cốt Lõi Của YOLO11s

1. **Khối C3k2 (Cross Stage Partial with 2/3 Convolutions)**:
   Thay thế khối C2f của thế hệ YOLOv8 trước đó. C3k2 sử dụng cấu trúc chia đôi luồng đặc trưng (Split & Merge) kết hợp kernel $3 \times 3$ tối ưu hóa tensor, giúp giảm 15% số phép tính FLOPS trong khi tăng tốc độ trích xuất đặc trưng biên dạng của biển báo.

2. **Khối SPPF (Spatial Pyramid Pooling - Fast)**:
   Thực hiện Max-Pooling liên tiếp với kernel $5 \times 5, 9 \times 9, 13 \times 13$, cho phép mạng mở rộng trường thụ cảm (Receptive Field) để nhận diện các biển báo có kích thước đa dạng từ $20\text{ px}$ đến $400\text{ px}$.

3. **Khối C2PSA (Cross Stage Partial with Pointwise Spatial Attention)**:
   Tích hợp cơ chế tự chú ý không gian (Self-Attention Mechanism) tại tầng sâu nhất của Backbone. Khối này tính ma trận trọng số chú ý không gian $A = \text{Softmax}\left(\frac{Q K^T}{\sqrt{d_k}}\right) V$, giúp mạng tập trung vào các vùng ảnh có biển báo nhỏ ở xa và bỏ qua các vùng hậu cảnh phức tạp (như tán lá cây hoặc tòa nhà cao tầng).

4. **Decoupled Anchor-Free Head**:
   Tách biệt hoàn toàn việc dự đoán tọa độ hộp và dự đoán nhãn lớp, loại bỏ các neo hộp (Anchor Boxes) cố định, giúp mô hình linh hoạt với các tỷ lệ khung hình biến thiên của biển báo.

---

### 3.2. Hệ Thống Hàm Mất Mát Toán Học (Loss Functions)

Tổng mất mát trong quá trình tối ưu hóa mạng YOLO11s được định nghĩa bởi:

$$\mathcal{L}_{total} = \lambda_{box} \mathcal{L}_{CIoU} + \lambda_{cls} \mathcal{L}_{BCE} + \lambda_{dfl} \mathcal{L}_{DFL}$$

Trong đó các siêu tham số chuẩn: $\lambda_{box} = 7.5$, $\lambda_{cls} = 0.5$, $\lambda_{dfl} = 1.5$.

#### A. Hàm mất mát hộp Complete IoU ($\mathcal{L}_{CIoU}$)
Đo lường độ trùng khớp không gian, khoảng cách tâm và tính nhất quán tỷ lệ khung hình giữa hộp dự đoán $b = [x, y, w, h]$ và hộp ground truth $b^{gt} = [x^{gt}, y^{gt}, w^{gt}, h^{gt}]$:
...
- $\rho(b, b^{gt}) = \sqrt{(x - x^{gt})^2 + (y - y^{gt})^2}$: Khoảng cách Euclid giữa 2 tâm hộp.
- $c$: Chiều dài đường chéo của hình chữ nhật bao quanh nhỏ nhất chứa cả 2 hộp.
- $v$: Tham số đo lường tính nhất quán của tỷ lệ khung hình (Aspect Ratio Consistency): $v = \frac{4}{\pi^2} \left( \arctan\frac{w^{gt}}{h^{gt}} - \arctan\frac{w}{h} \right)^2$.
- $\alpha$: Trọng số cân bằng: $\alpha = \frac{v}{(1 - IoU) + v}$.

#### B. Hàm mất mát phân phối hồi quy Distribution Focal Loss ($\mathcal{L}_{DFL}$)
Thay vì dự đoán một giá trị tọa độ đơn lẻ (dễ bị ảnh hưởng bởi viền mờ của biển), DFL dự đoán một phân phối xác suất liên tục quanh giá trị thực tế $y \in [y_i, y_{i+1}]$:

$$\mathcal{L}_{DFL}(\mathcal{S}_i, \mathcal{S}_{i+1}) = - \left( (y_{i+1} - y) \log(\mathcal{S}_i) + (y - y_i) \log(\mathcal{S}_{i+1}) \right)$$

Tọa độ dự đoán cuối cùng là tích phân kỳ vọng: $\hat{y} = \sum_{i=0}^n \text{Softmax}(\mathcal{S}_i) \cdot i$.

#### C. Hàm mất mát phân loại Binary Cross Entropy ($\mathcal{L}_{BCE}$)
$$\mathcal{L}_{BCE}(p, y) = - \left( y \log(p) + (1 - y) \log(1 - p) \right)$$

---

### 3.3. Thuật Toán Lọc Trùng Lặp NMS & Soft-NMS

Sau khi mạng xuất ra hàng nghìn hộp dự đoán ứng viên, thuật toán loại bỏ hộp thừa Non-Maximum Suppression (NMS) hoặc Soft-NMS được kích hoạt:

```text
Thuật toán Soft-NMS với hàm suy giảm Gaussian:
Input: Tập hộp B = {b1, ..., bn}, tập điểm S = {s1, ..., sn}, ngưỡng Nt, tham số sigma.
Output: Tập hộp cuối cùng D, tập điểm tương ứng S_final.

1. Khởi tạo D = []
2. While B không rỗng:
     m = argmax(S)
     M = B[m]
     D = D + [M]
     B = B \ {M}
     For bi in B:
         iou = IoU(M, bi)
         si = si * exp(- (iou^2) / sigma)   # Hàm suy giảm trọng số mượt
     Loại bỏ các hộp có si < score_threshold
3. Return D, S
```

---

## 4. Chi Tiết Nhánh Computer Vision Truyền Thống (Classical CV)

Nhánh Computer Vision truyền thống chạy song song trên CPU, hoạt động như một lớp **kiểm chứng độc lập (Independent Verification)** nhằm bảo vệ hệ thống trước các lỗi ảo giác (Hallucination) hoặc bỏ sót bất thường của mạng nơ-ron:

```mermaid
flowchart LR
    A["Frame BGR"] --> B["Chuyển sang HSV"]
    B --> C["Lọc Ngưỡng Màu (In-Range Mask)"]
    C --> D["Phép Toán Hình Thái Học (Morphology)"]
    D --> E["Tìm Đề Cử Đường Tròn (Hough Circles)"]
    E --> F["Phân Tích Đường Bao & Độ Tròn (Circularity)"]
    F --> G["Candidate Heuristic Score"]
```

### 4.1. Công Thức Toán Học Chuyển Đổi Không Gian Màu BGR Sang HSV
Ảnh màu BGR được chuyển đổi sang không gian màu HSV (Hue - Sắc độ, Saturation - Độ bão hòa, Value - Giá trị độ sáng) để tách biệt thông tin màu sắc khỏi cường độ ánh sáng:

$$V = \max(R, G, B), \quad \Delta = V - \min(R, G, B)$$

$$S = \begin{cases} 0 & \text{nếu } V = 0 \\ \frac{\Delta}{V} & \text{nếu } V > 0 \end{cases}$$

$$H = \begin{cases} 0^\circ & \text{nếu } \Delta = 0 \\ 60^\circ \times \left( \frac{G - B}{\Delta} \pmod 6 \right) & \text{nếu } V = R \\ 60^\circ \times \left( \frac{B - R}{\Delta} + 2 \right) & \text{nếu } V = G \\ 60^\circ \times \left( \frac{R - G}{\Delta} + 4 \right) & \text{nếu } V = B \end{cases}$$

Quy chuẩn dải màu trong OpenCV ($H \in [0, 180]$, $S, V \in [0, 255]$) cho biển báo Việt Nam theo **QCVN 41:2019**:

- **Màu đỏ (Biển cấm / nguy hiểm)**: Do kênh Hue đỏ nằm ở 2 đầu dải quang phổ, ta dùng hợp của 2 dải: $\text{Red}_1: H \in [0, 10], S \in [70, 255], V \in [50, 255]$ và $\text{Red}_2: H \in [170, 180], S \in [70, 255], V \in [50, 255]$.
- **Màu xanh dương (Biển hiệu lệnh R-series)**: $H \in [100, 130], S \in [80, 255], V \in [50, 255]$.
- **Màu vàng (Biển cảnh báo W-series)**: $H \in [20, 35], S \in [80, 255], V \in [50, 255]$.

### 4.2. Các Phép Toán Hình Thái Học (Mathematical Morphology)
Mặt nạ nhị phân sau khi lọc màu được làm sạch bằng phần tử cấu trúc hình đĩa $B$ (Structuring Element kích thước $3 \times 3$ hoặc $5 \times 5$):

- **Phép mở (Opening)** để khử các đốm nhiễu hạt nhỏ do lá cây hoặc đèn đường: $A \circ B = (A \ominus B) \oplus B$.
- **Phép đóng (Closing)** để hàn gắn các vết nứt, đứt đoạn trên viền tròn của biển báo do đọng nước hoặc bóng râm: $A \bullet B = (A \oplus B) \ominus B$.

### 4.3. Thuật Toán Biến Đổi Hough Tìm Đường Tròn (Hough Circle Transform)
Phương trình quỹ đạo đường tròn tâm $(a, b)$, bán kính $r$:

$$(x - a)^2 + (y - b)^2 = r^2 \Rightarrow \begin{cases} a = x - r \cos\theta \\ b = y - r \sin\theta \end{cases}$$

Thuật toán sử dụng hướng vector Gradient $\nabla I = \left[\frac{\partial I}{\partial x}, \frac{\partial I}{\partial y}\right]^T$ tại mỗi điểm biên Canny để tích lũy phiếu bầu trong không gian tham số 3 chiều $(a, b, r)$, giúp giảm độ phức tạp tính toán từ $O(N^3)$ xuống $O(N^2)$.

### 4.4. Bộ Mô Tả Hình Học (Contour Shape Descriptors)
- **Độ tròn (Form Factor / Circularity - $\mathcal{C}$)**:

  $$\mathcal{C} = \frac{4 \pi \cdot \text{Area}}{\text{Perimeter}^2}$$

  - Hình tròn lý thuyết: $\mathcal{C} = 1.0$.
  - Biển báo tròn Việt Nam thực tế (sau biến dạng chiếu phối cảnh góc nghiêng): $\mathcal{C} \in [0.85, 1.05]$.
  - Hình vuông: $\mathcal{C} = \frac{\pi}{4} \approx 0.785$.
  - Hình tam giác đều (biển cảnh báo): $\mathcal{C} = \frac{\pi \sqrt{3}}{9} \approx 0.605$.
- **Tỷ lệ khung hình (Aspect Ratio - $AR$)**:

  $$AR = \frac{Width}{Height} \in [0.85, 1.15]$$

---

#### 4.5. Động Cơ Hợp Nhất Quyết Định Hybrid (Bayesian Confidence Fusion Engine)

Khi mô hình YOLO11s gặp khó khăn do thời tiết mù mờ khiến điểm tin cậy rớt xuống vùng ranh giới ($0.15 \le \text{conf}_{yolo} < 0.35$), hệ thống kích hoạt thuật toán hợp nhất Bayes:

$$P_{fused} = \min\left(1.0, w_{yolo} \cdot \text{conf}_{yolo} + w_{cv} \cdot \text{score}_{cv} + \text{Bonus}\right)$$

Trong đó:

- $\text{score}_{cv} = 0.5 \cdot \mathcal{C}_{normalized} + 0.3 \cdot \text{ColorMatch} + 0.2 \cdot \text{HoughAlignment}$.
- Trọng số chuẩn: $w_{yolo} = 0.60, w_{cv} = 0.40$.
- $\text{Bonus} = +0.15$ nếu tâm đường tròn Hough trùng với tâm hộp YOLO trong vòng bán kính $\le 5\text{ pixels}$.

#### Bảng Quy Tắc Hành Xử Hợp Nhất (Hybrid Decision Matrix)

| Điểm YOLO ($\text{conf}_{yolo}$) | Kết Quả Nhánh Heuristic CV | Quyết Định Trọng Tài | Hành Động Pipeline |
|:---:|:---:|:---:|:---|
| $\ge 0.50$ | Bất kỳ | **Chấp nhận YOLO** | Đưa thẳng vào Multi-Object Tracker. |
| $0.15 \le \text{conf} < 0.50$ | Khớp màu & độ tròn cao ($\text{score}_{cv} \ge 0.60$) | **Nâng điểm Bayes** | $P_{fused} \ge 0.50 \rightarrow$ Đưa vào Tracker. |
| $0.15 \le \text{conf} < 0.50$ | Không tìm thấy hình dạng ($\text{score}_{cv} < 0.30$) | **Từ chối (Suppress)** | Bỏ qua, coi là False Alarm của YOLO. |
| $< 0.15$ | Độ tròn hoàn hảo ($\text{score}_{cv} \ge 0.85$) | **Tạo Heuristic Tracklet** | Đưa vào Tracker với nhãn `Candidate (CV)`. |

#### Ví Dụ Số Học Thực Nghiệm: Giải Cứu Biển Báo P.127 Bị Bỏ Sót

1. **Quan sát YOLO**: Biển báo $60\text{ km/h}$ bị chói đèn pha, điểm tin cậy suy giảm chỉ còn $\text{conf}_{yolo} = 0.22$.
2. **Quan sát Heuristic CV**:
   - Tỷ lệ màu đỏ viền: $\text{ColorMatch} = 0.90$.
   - Độ tròn biến dạng nhẹ: $\mathcal{C} = 0.94 \rightarrow \mathcal{C}_{normalized} = 0.94$.
   - Tâm Hough lệch $2\text{ pixels}$ ($\le 5\text{ px}$) $\rightarrow \text{Bonus} = +0.15, \text{HoughAlignment} = 0.90$.
   - Điểm CV: $\text{score}_{cv} = 0.5 \times 0.94 + 0.3 \times 0.90 + 0.2 \times 0.90 = 0.470 + 0.270 + 0.180 = 0.920$.
3. **Tính toán hợp nhất**:

   $$P_{fused} = 0.60 \times 0.22 + 0.40 \times 0.92 + 0.15 = 0.132 + 0.368 + 0.15 = 0.650$$

4. **Kết luận**:
   Điểm tin cậy được nâng từ **$0.22$ lên $0.65$** ($\ge 0.50$). Biển báo được giải cứu thành công, chuyển sang `RobustTSRTracker` để đưa lên taplo cảnh báo cho tài xế trước $2.5\text{ giây}$!

---

## 5. So Sánh Các Kiến Trúc Nhận Thức TSR Tiên Tiến Trên Thế Giới

Để hệ thống TSR đạt chuẩn an toàn chức năng và đáp ứng yêu cầu vận hành thực tế (theo phân tích dừng xe an toàn $SSD$ tại [0.requirements.md](../0.requirements.md)), các nhóm kỹ sư ADAS trên thế giới và các nền tảng tự hành như **Autoware**, **Apollo**, hay các nghiên cứu từ cộng đồng quốc tế đã phát triển nhiều trường phái kiến trúc khác nhau. Dưới đây là phân tích và đối sánh chuyên sâu giữa 3 mô hình nhận thức chủ đạo:

### 5.1. Ba Mô Hình Nhận Thức Biển Báo Cốt Lõi

```mermaid
flowchart TD
    subgraph M1["Mô Hình 1: Single-Stage 82 Classes (Baseline Hiện Tại)"]
        A1["Input 1080p/4K"] --> B1["Downscale 640x640"]
        B1 --> C1["YOLOv8/11 (82 classes)"]
        C1 --> D1["Direct Output (BBox + Class)"]
    end

    subgraph M2["Mô Hình 2: Two-Stage (Coarse Detector + Tiny OCR/Classifier)"]
        A2["Input 1080p/4K"] --> B2["Downscale 640x640"]
        B2 --> C2["Lightweight Detector (4 Siêu Lớp Hình Học)"]
        C2 --> D2["Crop BBox ROI (Native Resolution)"]
        D2 --> E2["MobileNetV3 / Tiny OCR (Đọc Số & Ký Hiệu)"]
    end

    subgraph M3["Mô Hình 3: Adaptive EROI / Slicing (SAHI-Lite)"]
        A3["Input 1080p/4K"] --> B3["Cắt Vùng Chân Trời EROI Lề Đường"]
        B3 --> C3["Tile 1 (Lề Phải 640x640)"]
        B3 --> D3["Tile 2 (Giá Long Môn 640x640)"]
        C3 --> E3["Detector High-Res"]
        D3 --> E3
        E3 --> F3["NMS Tiled Merging"]
    end
```

#### Mô hình 1: Single-Stage End-to-End Detector (Baseline repo)
*   **Nguyên lý:** Sử dụng một mạng nơ-ron duy nhất (YOLOv8s/YOLO11s) nhận đầu vào ảnh toàn cảnh đã thu nhỏ (ví dụ $640 \times 640$), đồng thời dự đoán bounding box và phân loại trực tiếp toàn bộ 82 lớp biển báo Việt Nam.
*   **Ưu điểm:** Pipeline đơn giản, chỉ cần một lần forward pass duy nhất; độ trễ suy luận ổn định và dễ đóng gói.
*   **Nhược điểm:**
    *   *Mất chi tiết ở cự ly xa:* Khi resize ảnh từ 1080p/4K về 640, biển báo ở khoảng cách $60 - 80\text{ m}$ chỉ còn kích thước $8 - 14\text{ pixels}$, dễ bị các tầng pooling/stride 32 của backbone làm biến mất hoàn toàn.
    *   *Nhầm lẫn giữa các lớp tương đồng:* Việc phân tách 82 lớp với nhiều biển có hình dạng giống hệt nhau (ví dụ: Biển tròn viền đỏ giới hạn tốc độ 50, 60, 80; biển cấm rẽ trái vs cấm quay đầu) chỉ dựa trên feature map thu nhỏ dẫn đến xác suất nhầm số rất cao.

#### Mô hình 2: Two-Stage Coarse Detection + Fine Classification / OCR (Chuẩn Công Nghiệp Autoware)
*   **Nguyên lý:** Phỏng theo kiến trúc của Autoware Universe (`traffic_light_classifier`) và các nghiên cứu của Đức (GTSRB/GTSDB):
    *   *Giai đoạn 1 (Geometry Detector):* YOLO chỉ cần phát hiện 4–5 siêu nhóm hình học cơ bản: Tròn đỏ (Cấm/Tốc độ), Tam giác vàng (Nguy hiểm), Tròn xanh (Hiệu lệnh), Vuông/Chữ nhật (Chỉ dẫn).
    *   *Giai đoạn 2 (Fine Classifier / OCR):* Khi phát hiện biển tròn đỏ, hệ thống crop ROI từ ảnh gốc ở độ phân giải cao và đưa qua mạng phân loại siêu nhẹ (MobileNetV3 hoặc mạng OCR nhận diện số như CRNN/SVTR-tiny).
*   **Ưu điểm:** Độ chính xác đọc số tốc độ ($50, 60, 80, 100, 120\text{ km/h}$) đạt $>98\%$. Giải quyết triệt để bài toán mất cân bằng dữ liệu (Long-tail classes) vì classifier giai đoạn 2 có thể train riêng biệt bằng ảnh crop tổng hợp hoặc data augmentation mạnh.
*   **Nhược điểm:** Pipeline phụ thuộc vào 2 mạng nơ-ron; nếu giai đoạn 1 bỏ sót biển (False Negative) thì giai đoạn 2 hoàn toàn không có cơ hội xử lý.

#### Mô hình 3: Adaptive EROI Slicing (SAHI - Slicing Aided Hyper Inference)
*   **Nguyên lý:** Thay vì cắt toàn bộ ảnh thành lưới như SAHI truyền thống (gây nổ số lượng inference), phương pháp **Adaptive EROI** chỉ cắt 1–2 tile ở vùng không gian có xác suất xuất hiện biển báo cao nhất:
    *   Vùng lề đường phía trước bên phải (theo luật đi bên phải của Việt Nam).
    *   Vùng giá long môn trên cao (khoảng $20\% - 45\%$ chiều cao ảnh).
*   **Ưu điểm:** Tăng cự ly phát hiện biển ($D_{detect}$) lên gấp **2.0 – 2.5 lần** (từ $35\text{ m}$ lên tới $80 - 110\text{ m}$), thỏa mãn trọn vẹn bài toán khoảng cách dừng xe $SSD$ ở dải vận tốc cao tốc $100 - 120\text{ km/h}$ trong [0.requirements.md](../0.requirements.md).
*   **Nhược điểm:** Tăng thêm 1–2 lần forward pass, đòi hỏi phần cứng biên phải có GPU/NPU đủ mạnh hoặc chạy dưới dạng TensorRT FP16.

### 5.2. Bảng Đối Sánh Toàn Diện Giữa Các Phương Pháp

| Tiêu Chí So Sánh | 1. Single-Stage (Hiện Tại) | 2. Two-Stage (Detect + OCR) | 3. Adaptive EROI (SAHI-Lite) |
|---|---|---|---|
| **Cự ly phát hiện xa ($D_{detect}$)** | Thấp ($25 - 40\text{ m}$) | Trung bình ($35 - 50\text{ m}$) | **Rất cao ($80 - 120\text{ m}$)** |
| **Độ chính xác đọc số tốc độ** | Trung bình ($70 - 82\%$) | **Rất cao ($>98\%$)** | Khá ($85 - 90\%$) |
| **Độ phức tạp triển khai** | **Thấp nhất (1 model duy nhất)** | Trung bình (2 model nối tầng) | Trung bình (cần logic crop & merge NMS) |
| **Độ trễ suy luận (CPU x86)** | $\approx 45 - 65\text{ ms}$ | $\approx 55 - 75\text{ ms}$ | $\approx 90 - 130\text{ ms}$ |
| **Độ trễ suy luận (GPU TensorRT FP16)**| $\approx 6 - 8\text{ ms}$ | $\approx 8 - 11\text{ ms}$ | $\approx 12 - 16\text{ ms}$ |
| **Khả năng xử lý Long-tail Class** | Kém (phụ thuộc dataset 82 class) | **Rất tốt (train độc lập classifier)**| Kém (vẫn dùng chung backbone) |
| **Tuân thủ bài toán SSD ($120\text{ km/h}$)**| Không đạt ở vận tốc cao | Đạt ở vận tốc trung bình ($<80\text{ km/h}$)| **Đạt chuẩn cao tốc ($\ge 120\text{ km/h}$)** |

### 5.3. Danh Mục Nguồn Tham Chiếu Chuẩn (Verifiable References)

1. **[YOLO Architecture]** Redmon, J., Divvala, S., Girshick, R., & Farhadi, A. (2016). *You Only Look Once: Unified, Real-Time Object Detection*. IEEE CVPR. [arXiv:1506.02640](https://arxiv.org/abs/1506.02640).
2. **[SAHI Framework]** Akyon, F. C., Altinuc, S. O., & Temizel, A. (2022). *Slicing Aided Hyper Inference and Fine-tuning for Small Object Detection*. IEEE International Conference on Image Processing (ICIP). [arXiv:2202.06934](https://arxiv.org/abs/2202.06934).
3. **[Autoware Universe Perception]** Autoware Foundation (2024). *Autoware Universe: Traffic Light and Traffic Sign Classification Module*. [GitHub Repo](https://github.com/autowarefoundation/autoware_universe).
4. **[GTSRB Benchmark]** Stallkamp, J., Schlipsing, M., Salmen, J., & Igel, C. (2012). *Man vs. computer: Benchmarking machine learning algorithms for traffic sign recognition*. Neural Networks, 32, 323–332. [DOI: 10.1016/j.neunet.2012.02.016](https://doi.org/10.1016/j.neunet.2012.02.016) | [Benchmark Portal](https://benchmark.ini.rub.de/).
5. **[Vietnamese Road Sign Regulation]** Bộ Giao thông Vận tải (2019). *Quy chuẩn kỹ thuật quốc gia về báo hiệu đường bộ (QCVN 41:2019/BGTVT)*. [🔗 thuvienphapluat.vn](https://thuvienphapluat.vn/van-ban/Giao-thong-Van-tai/Thong-tu-54-2019-TT-BGTVT-Quy-chuan-ky-thuat-quoc-gia-ve-bao-hieu-duong-bo-433431.aspx)
6. **[Expressway Design Standard]** Tiêu chuẩn Quốc gia (2012). *Đường ô tô cao tốc - Yêu cầu thiết kế (TCVN 5729:2012)*. [🔗 tieuchuan.vsqi.gov.vn](https://tieuchuan.vsqi.gov.vn/tieuchuan/view?id=12555)

---

## 6. Kiến Trúc Xử Lý Bất Đồng Bộ Đa Luồng (Producer-Consumer Multi-threading)

Runtime hiện tại đã được nâng cấp hỗ trợ chế độ đa luồng bất đồng bộ qua lớp `ThreadedVideoCapture`. Pipeline tách biệt các vai trò để tránh hiện tượng tắc nghẽn I/O:
...
flowchart LR
    ...
    classDef layerState fill:#E8F5E9,stroke:#43A047,stroke-width:2px,color:#1B5E20
    classDef layerOutput fill:#FCE4EC,stroke:#D81B60,stroke-width:2px,color:#880E4F
    classDef nodeQueue fill:#FFF9C4,stroke:#FBC02D,stroke-width:2px,stroke-dasharray: 5 5,color:#F57F17

    T1["Luồng 1: ThreadedVideoCapture (Camera/Video)"]:::layerSensing --> Q1["Frame Queue (depth=1 cho Live, 8 cho File)"]:::nodeQueue
    Q1 --> T2["Luồng 2: YOLO Inference Worker (ONNX/TRT)"]:::layerPerception
    T2 --> Q2["Detection Queue"]:::nodeQueue
    Q2 --> T3["Luồng 3: RobustTSRTracker (ByteTrack + Voting)"]:::layerState
    T3 --> T4["Luồng 4: Overlay & CAN/HMI Writer"]:::layerOutput
```

Nguyên tắc bất đồng bộ:

- Với live camera/webcam, `queue_size = 1`: tự động drop frame cũ để đảm bảo zero latency lag (không bao giờ tích tụ độ trễ).
- Với file video replay, `queue_size = 8`: giải mã trước để đảm bảo GPU/NPU luôn có dữ liệu liên tục, không bị starvation.
- Ghi file video và hiển thị GUI chạy tách biệt, không chặn chu kỳ suy luận của mạng nơ-ron.

---

## 7. SOTIF Traceability & An Toàn Nhận Thức (ISO 21448)

| Rủi ro SOTIF | Cách pipeline hỗ trợ | Hiện trạng kiểm chứng |
|---|---|---|
| YOLO miss biển rõ | Traditional CV tạo candidate đối chiếu & nâng điểm Bayes. | Đã kiểm chứng trong `code/tsr_demo.py` (`verify_low_confidence_yolo`). |
| Heuristic false alarm | YOLO và NMS lọc candidate yếu; Temporal Voting lọc false alarm tức thời. | Đã kiểm chứng trong `code/tracker.py` (cần $\ge 3$ frame xác nhận). |
| Camera mưa mờ / chói lóa | IEEE 2020 CTA monitor phát hiện và đưa vào degraded/unavailable mode. | Đã kiểm chứng trong `code/cta_monitor.py` (`IEEE2020CTAMonitorV2`). |
| HMI flicker & nhảy class | ByteTrack duy trì ID và Temporal Majority Voting ổn định nhãn. | Đã kiểm chứng trong `code/tracker.py` (loại bỏ 100% hiện tượng chớp tắt). |

---

## 8. Acceptance Criteria & Tiêu Chí Nghiệm Thu

- Pipeline chạy được với YOLO-only, hybrid mode, và ByteTrack tracker.
- Detection output giữ được `source`, `bbox`, `class`, `confidence`, `track_id`, `is_confirmed`.
- Nhánh heuristic không được publish như ground truth; nó chỉ là candidate hoặc cross-check.
- Latency từng block có thể đo được (tách bạch $T_{read}, T_{prep}, T_{infer}, T_{track}, T_{hmi}$).
- Output video demo không được gọi là HMI production thật; nó chỉ là visual telemetry phục vụ kiểm thử.

---

---

## 9. Phân Tích Chuyên Sâu Kiến Trúc Detector Cho Automotive TSR (Detector Architecture Deep Reference)
...
| 3 | [Tan et al., *EfficientDet*](https://arxiv.org/abs/1911.09070) | BiFPN and efficient compound scaling. | Accuracy/latency trade-off cho embedded TSR. |
...
| 12 | [GTSRB / GTSDB Benchmark](https://benchmark.ini.rub.de/) | German classification/detection benchmark pages. | Differentiating recognition vs detection benchmark roles. |
...
| 14 | [Ultralytics Export / Benchmark Guide](https://docs.ultralytics.com/modes/export/) | Export workflow and backend comparison entrypoint. | ONNX/OpenVINO/TensorRT export path. |
| 15 | [ONNX Runtime Documentation](https://onnxruntime.ai/docs/) | ONNX inference runtime. | Portable edge/runtime deployment option. |
| 16 | [NVIDIA TensorRT Documentation](https://docs.nvidia.com/deeplearning/tensorrt/) | NVIDIA optimized inference runtime. | GPU optimized deployment and INT8/FP16 discussion. |
| 17 | [Intel OpenVINO Documentation](https://docs.openvino.ai/) | Intel optimized inference toolkit. | CPU/iGPU edge deployment option. |


---

## 10. Đánh Giá Hiện Trạng Baseline Repo & Kế Hoạch Sản Xuất Hóa (Baseline Repo Audit & Production Readiness)
