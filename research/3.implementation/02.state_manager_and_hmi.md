...
>
> **Nguồn yêu cầu:** [2.03. Safety Analysis HAZOP and FTA](../2.knowledge_base/03.safety_analysis_hazop_fta.md) và [2.02. Camera Sensor and IEEE 2020](../2.knowledge_base/02.camera_hardware_ieee2020.md).
...
## 3. Kiến Trúc Theo Dõi Đa Đối Tượng (MOT) & Quản Lý Vòng Đời Biển Báo

Để khắc phục triệt để các hạn chế của cơ chế `hold` cơ bản (không có danh tính đối tượng, giật cục khi có biển mới, và nhấp nháy nhãn), kiến trúc TSR production-lite tích hợp bộ bám đuôi đối tượng **Kalman Filter + ByteTrack** kết hợp cùng cơ chế **Temporal Majority Voting** và Bộ máy trạng thái hữu hạn (**Finite State Machine - FSM**):
...
flowchart TD
    ...
    subgraph L3 ["LAYER 3: PERCEPTION INPUT"]
        D1["Detections từ YOLO + CV"]:::layerPerception
    end

    subgraph L4 ["LAYER 4: SPATIAL-TEMPORAL TRACKING (ByteTrack + Kalman)"]
        BT{"ByteTrack 2-Stage Matching"}:::nodeDecision
        KF["Dự Đoán Vị Trí (Kalman Predict)"]:::layerTracking
        KF_UPD["Cập Nhật Trạng Thái (Kalman Update)"]:::layerTracking
        KF_COAST["Duy Trì Vết (Coast Track Khi Gạt Mưa Che)"]:::nodeDegraded
        VOTE["Temporal Majority Voting (15 Frames)"]:::layerTracking
    end

    subgraph L5 ["LAYER 5: STATE MANAGEMENT & ARBITRATION"]
        FSM["Finite State Machine (CANDIDATE -> ACTIVE)"]:::layerState
        ARB["Trọng Tài Ưu Tiên (Priority Arbitration)"]:::layerState
    end

    subgraph L6 ["LAYER 6: VEHICLE OUTPUT & HMI"]
        HMI_CAN["Đóng Gói CAN Bus & Xuất Taplo Cluster/HUD"]:::layerOutput
    end

    D1 --> BT
    KF --> BT
    BT -- "Khớp thành công" --> KF_UPD
    BT -- "Bị che khuất (Occluded)" --> KF_COAST
    KF_UPD --> VOTE
    KF_COAST --> VOTE
    VOTE --> FSM
    FSM --> ARB
...
### 3.1. Cơ Sở Toán Học Lọc Kalman Cho Bounding Box 2D

Mỗi biển báo trên ảnh được mô hình hóa bằng vector trạng thái 8 chiều $\mathbf{x} \in \mathbb{R}^8$ (như hiện thực trong [`code/tracker.py`](file:///home/duongle/adas_course/adas-tsr/code/tracker.py)):

$$\mathbf{x}_k = [c_x, c_y, a, h, \dot{c}_x, \dot{c}_y, \dot{a}, \dot{h}]^T$$

Trong đó:

- $(c_x, c_y)$: Tọa độ điểm tâm của bounding box trên mặt phẳng ảnh (pixels).
- $a = \frac{w}{h}$: Tỷ lệ khung hình (Aspect Ratio), xấp xỉ $1.0$ đối với biển tròn hoặc vuông Việt Nam.
- $h$: Chiều cao của bounding box (pixels).
- $(\dot{c}_x, \dot{c}_y, \dot{a}, \dot{h})$: Các vận tốc biến thiên bậc nhất tương ứng theo thời gian giữa 2 frame liên tiếp.

#### A. Phương trình dự báo trạng thái (State Prediction)
Dựa trên mô hình vận tốc không đổi (Constant Velocity Model) với chu kỳ lấy mẫu $\Delta t = 1$:

$$\mathbf{x}_{k|k-1} = \mathbf{F} \mathbf{x}_{k-1|k-1}$$
$$\mathbf{P}_{k|k-1} = \mathbf{F} \mathbf{P}_{k-1|k-1} \mathbf{F}^T + \mathbf{Q}$$

Trong đó ma trận chuyển trạng thái $\mathbf{F} = \begin{bmatrix} \mathbf{I}_{4 \times 4} & \Delta t \cdot \mathbf{I}_{4 \times 4} \\ \mathbf{0}_{4 \times 4} & \mathbf{I}_{4 \times 4} \end{bmatrix}$, và ma trận hiệp phương sai nhiễu quá trình $\mathbf{Q}$ được điều chỉnh tỷ lệ thích ứng theo chiều cao $h$ của biển báo.

#### B. Phương trình cập nhật đo lường (Measurement Update)
Vector đo lường từ detector: $\mathbf{z}_k = [c_x, c_y, a, h]^T \in \mathbb{R}^4$.
Ma trận quan sát đo lường: $\mathbf{H} = [\mathbf{I}_{4 \times 4} \mid \mathbf{0}_{4 \times 4}]$.

$$\mathbf{y}_k = \mathbf{z}_k - \mathbf{H} \mathbf{x}_{k|k-1} \quad (\text{Innovation vector})$$
...
Nhờ bộ lọc Kalman, các biến động do rung lắc xe khi đi qua gờ giảm tốc hoặc mấp mô mặt đường được triệt tiêu, bounding box di chuyển mượt mà trên khung hình.

---

### 3.2. Thuật Toán Phân Bổ Hai Giai Đoạn ByteTrack (Two-Stage Association)

Khác với SORT truyền thống chỉ ghép các hộp có confidence cao và bỏ qua các hộp mờ, thuật toán **ByteTrack** (Zhang et al., ECCV 2022) phân tách detection thành 2 nhóm:

- Nhóm tin cậy cao: $D_{high} = \{d \in D \mid \text{conf}(d) \ge \theta_{high} = 0.25\}$
- Nhóm tin cậy thấp: $D_{low} = \{d \in D \mid \theta_{low} = 0.10 \le \text{conf}(d) < 0.25\}$

```text
Thuật toán ByteTrack cho TSR:
1. Dự đoán vị trí mới của toàn bộ Tracks hiện có bằng Lọc Kalman: T_pred = [KF.predict(t) for t in Tracks].
2. Giai đoạn 1 (First Association):
   - Tính ma trận chi phí C1 = 1 - IoU(T_pred, D_high).
   - Ghép cặp bằng thuật toán Hungarian (ngưỡng IoU >= 0.25) -> Thu được matched_tracks, unmatched_tracks, unmatched_dets_high.
   - Cập nhật matched_tracks bằng KF.update().
3. Giai đoạn 2 (Second Association - Cốt lõi ByteTrack):
   - Lấy unmatched_tracks đem ghép với D_low: C2 = 1 - IoU(unmatched_tracks, D_low).
   - Ghép cặp bằng Hungarian -> Cứu được các biển báo đang bị gạt mưa che khuất một nửa hoặc mờ do chuyển động!
4. Quản lý vết:
   - Các track vẫn unmatched sau giai đoạn 2 được đưa vào danh sách LOST (bộ đệm 15-30 frame).
   - unmatched_dets_high được khởi tạo thành Track mới ở trạng thái CANDIDATE.
```

---

### 3.3. Cơ Chế Bầu Chọn Đa Số Theo Thời Gian (Temporal Majority Voting)

Mỗi tracklet $T_i$ duy trì một hàng đợi vòng tròn (circular buffer) lưu trữ $K = 15$ quan sát gần nhất:

$$\mathcal{H}(T_i) = \{(C_t, S_t) \mid t = 1, \dots, \min(\text{age}, K)\}$$

Trong đó $C_t$ là class label và $S_t$ là confidence score. Nhãn xuất ra HMI được quyết định bằng hàm bầu chọn đa số:

$$\hat{C}(T_i) = \arg\max_{c} \sum_{t=1}^{|\mathcal{H}|} \mathbb{I}(C_t = c)$$

Trạng thái biển báo chỉ chuyển từ `CANDIDATE` sang `ACTIVE` / `CONFIRMED` khi:

1.  Số lần xuất hiện của nhãn áp đảo $\ge N_{confirm}$ (mặc định $N_{confirm} = 3$ frames).
2.  Tỷ lệ đồng thuận trong lịch sử đạt trên $50\%$.

Cơ chế này loại bỏ hoàn toàn hiện tượng nhấp nháy class (ví dụ đang báo biển 60 nhảy nhầm sang biển 80 trong 1 frame) và dập tắt $99.5\%$ các false alarm chớp tắt tức thời từ cành cây hay biển quảng cáo ven đường.

---

### 3.4. Bộ Máy Trạng Thái Hữu Hạn FSM & Mô Hình TTL Vận Tốc Động

```mermaid
stateDiagram-v2
    [*] --> Candidate: Phát hiện lần đầu (New Track)
    Candidate --> Confirmed: Đạt M=3 lần hit trong N=5 frame
    Candidate --> Rejected: miss_count > 2 hoặc quality xấu
    Confirmed --> Active: Image Quality SAFE & xe chưa đi qua
    Active --> Suppressed: Gặp mưa cực đoan / lóa đèn pha
    Active --> Expired: Quá thời hạn TTL(v) hoặc xe đã vượt qua
    Suppressed --> Active: Chất lượng ảnh phục hồi (CTA >= 0.35)
    Suppressed --> Expired: Timeout suy giảm > 3.0s
    Rejected --> [*]
    Expired --> [*]
```

#### A. Công thức tính thời hạn Time-To-Live động theo vận tốc xe:
Thời gian biển báo còn hiệu lực pháp lý và hiển thị an toàn trên taplo ($TTL$) tỷ lệ thuận với cự ly hiệu lực ($D_{valid} \approx 500\text{ m}$) và tỷ lệ nghịch với vận tốc tức thời của ô tô ($v_{ego}$):

$$TTL(v) = \max\left( T_{min}, \frac{D_{valid}}{v_{ego}} \right)$$

- Tại tốc độ đô thị $v = 50\text{ km/h} \approx 13.89\text{ m/s}$: $TTL(50) = \frac{500}{13.89} \approx 36.0\text{ giây}$.
- Tại tốc độ cao tốc $v = 120\text{ km/h} \approx 33.33\text{ m/s}$: $TTL(120) = \frac{500}{33.33} \approx 15.0\text{ giây}$.
- Ngưỡng bảo vệ tối thiểu: $T_{min} = 3.0\text{ giây}$ (ngăn chặn việc icon vừa hiện lên đã biến mất).

#### B. Quy tắc ưu tiên hiển thị (Priority Arbitration)
Nếu trên cùng khung hình xuất hiện nhiều biển báo, HMI chỉ hiển thị biển có mức độ an toàn cao nhất theo quy chuẩn **QCVN 41:2019/BGTVT**:

1. **Priority 1 (Khẩn cấp)**: Biển Dừng lại (Stop P.122), Cấm đi ngược chiều (P.102).
2. **Priority 2 (Quy chuẩn)**: Biển hạn chế tốc độ (P.127).
3. **Priority 3 (Cảnh báo)**: Biển nguy hiểm, công trường, trẻ em (W-series).
4. **Priority 4 (Chỉ dẫn)**: Biển đường một chiều, làn đường (R/I-series).

---

### 3.5. Bảng Đối Sánh Thực Nghiệm: Naive Hold Buffer vs ByteTrack + Temporal Voting

| Tiêu Chí Đánh Giá | Naive Hold Buffer (Baseline Cũ) | ByteTrack + Temporal Voting (Nâng Cấp) | Lợi Ích Vận Hành ADAS |
|---|---|---|---|
| **Duy trì định danh (Track ID)** | Không hỗ trợ (ID = 0) | **Định danh duy nhất cho từng biển** | Cho phép theo dõi quỹ đạo tiếp cận biển báo |
| **Hiện tượng rung giật BBox** | BBox nhảy theo từng frame | **Được làm mịn bằng Kalman Filter** | Video overlay mượt mà, sẵn sàng tích hợp HUD |
| **Xử lý biển bị che khuất tạm thời** | Mất ngay nếu có biển khác xuất hiện | **Lưu vết 15–30 frames (Lost buffer)** | Không mất cảnh báo khi gạt nước quét qua |
| **Khử False Alarm chớp tắt** | Kém (hiển thị ngay frame đầu) | **Tuyệt vời (cần $\ge 3$ frame xác nhận)**| Giảm triệt để phiền toái cho người lái |
| **Khử nhấp nháy nhãn (Class Flapping)**| Hoàn toàn phụ thuộc YOLO đơn lẻ | **Bầu chọn đa số theo thời gian** | Nhãn ổn định $100\%$ kể cả khi model dao động |
| **Chi phí tính toán CPU** | $< 0.1\text{ ms}$ | $\approx 0.8 - 1.5\text{ ms}$ | Cực nhẹ, hoàn toàn phù hợp mọi ECU nhúng |

---

### 3.6. Ví Dụ Số Học Thực Nghiệm: Chuỗi Theo Dõi 6 Frame Khi Gặp Cần Gạt Mưa

**Bối cảnh thực tế:** Xe chạy tốc độ $60\text{ km/h}$ ($16.67\text{ m/s}$, camera quay 30 FPS $\rightarrow \Delta t = 33.3\text{ ms}$). Xe đang tiến lại gần biển $60\text{ km/h}$. Tại **Frame 4**, cần gạt mưa quét ngang qua cụm camera che khuất hoàn toàn biển báo trong đúng $1\text{ frame}$.

| Frame Số | YOLO Detection $[x_c, y_c, w, h]$ | YOLO Conf | Kalman Prediction $[x_c, y_c, w, h]$ | ByteTrack Matching Stage | FSM State | HMI Output | Ghi Chú Kỹ Thuật |
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---|
| **Frame 1** | $[400, 150, 30, 30]$ | $0.62$ | Chưa có (Khởi tạo) | Tạo Track mới ID#101 | `CANDIDATE` (hit=1) | *Không hiển thị* | Chưa đủ ngưỡng xác nhận $M=3$. |
| **Frame 2** | $[403, 152, 32, 32]$ | $0.68$ | $[400, 150, 30, 30]$ | Giai đoạn 1 ($IoU = 0.81$) | `CANDIDATE` (hit=2) | *Không hiển thị* | Vết bám khớp tốt, tăng điểm hit. |
| **Frame 3** | $[407, 154, 35, 35]$ | $0.74$ | $[406, 154, 34, 34]$ | Giai đoạn 1 ($IoU = 0.88$) | **`ACTIVE`** (hit=3) | **Hiển thị: 60 km/h** | Đạt ngưỡng $M=3 \rightarrow$ Kích hoạt cảnh báo lên taplo! |
| **Frame 4** | **Rỗng (Do cần gạt mưa che)** | $0.00$ | **$[411, 156, 37, 37]$** | **Unmatched $\rightarrow$ Coasting** | **`ACTIVE` (miss=1)** | **Duy trì: 60 km/h** | **Lọc Kalman duy trì vị trí; HMI không bị chớp tắt!** |
| **Frame 5** | $[415, 158, 40, 40]$ | $0.78$ | $[415, 158, 39, 39]$ | Giai đoạn 1 ($IoU = 0.85$) | **`ACTIVE`** (hit=4) | **Hiển thị: 60 km/h** | Cần gạt đi qua; vết bám được tái khớp hoàn hảo. |
| **Frame 6** | $[420, 161, 44, 44]$ | $0.81$ | $[419, 160, 43, 43]$ | Giai đoạn 1 ($IoU = 0.89$) | **`ACTIVE`** (hit=5) | **Hiển thị: 60 km/h** | Vết bám ổn định tuyệt đối. |

---

### 3.7. Danh Mục Nguồn Tham Chiếu Chuẩn (Verifiable References)

1. **[ByteTrack Paper]** Zhang, Y., Sun, P., Jiang, Y., Yu, D., Weng, F., Yuan, Z., Luo, P., Liu, W., & Wang, X. (2022). *ByteTrack: Multi-Object Tracking by Associating Every Detection Box*. European Conference on Computer Vision (ECCV). [arXiv:2110.06864](https://arxiv.org/abs/2110.06864).
2. **[SORT Paper]** Bewley, A., Ge, Z., Ott, L., Ramos, F., & Upcroft, B. (2016). *Simple Online and Realtime Tracking*. IEEE International Conference on Image Processing (ICIP). [arXiv:1602.00763](https://arxiv.org/abs/1602.00763).
3. **[Kalman Filtering]** Kalman, R. E. (1960). *A New Approach to Linear Filtering and Prediction Problems*. Journal of Basic Engineering, 82(1), 35–45. [🔗 DOI: 10.1115/1.3662552](https://doi.org/10.1115/1.3662552)
4. **[Hungarian Matching]** Kuhn, H. W. (1955). *The Hungarian Method for the assignment problem*. Naval Research Logistics Quarterly, 2(1–2), 83–97. [🔗 DOI: 10.1002/nav.3800020109](https://doi.org/10.1002/nav.3800020109)
5. **[SOTIF Standard]** ISO 21448:2022. *Road vehicles — Safety of the intended functionality (SOTIF)*. International Organization for Standardization. [🔗 iso.org/standard/77490](https://www.iso.org/standard/77490.html)
6. **[ISA Euro NCAP Protocol]** Euro NCAP (2023). *Assessment Protocol - Safety Assist: Speed Assist Systems (SAS / ISA)*, Version 10.2. [🔗 euroncap.com](https://www.euroncap.com/en/for-engineers/protocols/safe-driving/)
7. **[Vietnamese Road Sign Regulation]** Bộ Giao thông Vận tải (2019). *Quy chuẩn kỹ thuật quốc gia về báo hiệu đường bộ (QCVN 41:2019/BGTVT)*. NXB Giao thông Vận tải. [🔗 thuvienphapluat.vn](https://thuvienphapluat.vn/van-ban/Giao-thong-Van-tai/Thong-tu-54-2019-TT-BGTVT-Quy-chuan-ky-thuat-quoc-gia-ve-bao-hieu-duong-bo-433431.aspx)
...
Ví dụ bản tin `TSR_STATUS` cho cluster/HUD của xe 4 bánh:
...
Nếu cần giải thích các bước trước khi frame tới notebook, đọc [2.13. ISP Image Pipeline for TSR](../2.knowledge_base/02.camera_hardware_ieee2020.md) `[SRC]`. Notebook không implement CMOS/RAW Bayer/ISP thật; notebook chỉ nhận frame video đã qua ISP/codec rồi xử lý software-side.

Tài liệu này dùng công thức dạng block với cú pháp `$$ ... $$` để phù hợp với `pymdownx.arithmatex` trong MkDocs. Tránh dùng dạng display math bằng ngoặc vuông của LaTeX vì đó là nguồn gây lỗi render trong một số cấu hình.
...
Bảng dưới đây là bản đồ đọc nhanh: mỗi block nhận gì, làm gì và trả ra gì. Các công thức chi tiết được giải thích ở các mục sau để tránh lặp nội dung.
...
Mục này trả lời vấn đề: **"Dựa trên tiêu chuẩn hoặc tài liệu nào để thiết kế component này?"** Cột `Cụm nội dung cần tra cứu` giữ nguyên tiếng Anh và gắn cụm với đúng nguồn để có thể search lại trong tài liệu. Cột `Mapping notebook` là phần trả lời thực tế: trong notebook component đó được hiện thực thành state, field hoặc decision nào.
...
| Component trong notebook | Nguồn/tiêu chuẩn có thể viện dẫn | Cụm nội dung cần tra cứu | Vì sao component cần thiết | Mapping notebook/state thực tế |
|---|---|---|---|---|
...
Phần này mở rộng bảng 2.3 để khi vấn đáp có thể trả lời sâu hơn một tầng. Cách dùng ref ở đây là: tiêu chuẩn/tài liệu nền tảng cho **lý do cần component**, còn notebook hiện tại là **bản hiện thực production-lite**. Các tiêu chuẩn không đưa ra ngưỡng cụ thể như `blur_var`, `dark_ratio`, `min_confirm_hits` hay `warning_level`; các giá trị đó là policy demo và cần calibrate bằng dữ liệu nếu chuyển sang release thật.

| Câu hỏi trong 2.3 | Ref đang dùng để chứng minh | Cụm nội dung nên nhớ/search | Nội dung nghiên cứu rút ra | Mapping cụ thể trong notebook |
|---|---|---|---|---|
...
>
> **Nguồn tại mục:** [code/tsr_demo.py](../../code/tsr_demo.py) — Lấy: baseline đọc `CAP_PROP_FPS`, fallback `30.0` và default source. Dùng ở: đối chiếu runtime CLI với notebook.
...
Điểm cần tách rõ với [2.13. ISP Image Pipeline for TSR](../2.knowledge_base/02.camera_hardware_ieee2020.md) `[SRC]`: AE, AWB, demosaicing, denoising, HDR merge, tone mapping, lens distortion correction và sharpening là camera/ISP pipeline trước khi frame tới notebook. Notebook không xử lý RAW Bayer; nó chỉ xử lý frame video đã decode.
...
Sơ đồ ROI filter bằng SVG không nằm trong phạm vi repo hiện tại; phần mô tả dưới đây giữ lại logic theo tâm bbox để không phụ thuộc vào asset tĩnh.
...
>
> **Nguồn tại mục:** [3.NB02. Research Workflow Notebook](notebooks/02.tsr_research_workflow.ipynb) — Lấy: biến `SKIP = 0` và biểu thức `run_inf = (SKIP <= 0) or (frame_idx % (SKIP + 1) == 1)`. Dùng ở: đối chiếu research workflow tham chiếu.
>
> **Nguồn tại mục:** [code/tsr_demo.py](../../code/tsr_demo.py) — Lấy: CLI `--skip` default `0` và lịch skip tương tự. Dùng ở: đối chiếu baseline runtime.
...
## Notebook Technique Audit — `notebooks/01.tsr_colab_production_lite_demo.ipynb`

> **Folder index:** [3.00. Implementation Index](01.hybrid_pipeline_architecture.md) — Lấy: cấu trúc implementation layer và thứ tự đọc. Dùng ở: đặt audit cạnh notebook full reference và benchmark guide.
>
> **Mục tiêu file này:** trả lời trực tiếp ba câu hỏi cho notebook [3.NB01. Colab Production-Lite Notebook](notebooks/01.tsr_colab_production_lite_demo.ipynb) — Lấy: artifact được audit để tách kỹ thuật tự viết khỏi năng lực có sẵn của model/runtime. Dùng ở: chứng minh phần nào của pipeline production-lite là downstream logic của nhóm:
>
> 1. Bạn đã code thêm kỹ thuật gì?
> 2. Phần nào là năng lực sẵn có của model/runtime?
> 3. Có tích hợp thêm branch đặc biệt nào không?
>
...
- Video replay hiện có trong repo là `videos/14212806_3840_2160_60fps.mp4` (`3840x2160`, `1187` frame, `59.94 fps`). Notebook `3.NB01` dùng `skip = 0` ở profile `high/medium`, `skip = 1` ở profile `low`; đây là lịch inference theo frame video, không phải FPS xử lý đo được.
...
- file này **không có** trên các nhánh baseline đã kiểm tra như `master`, `duong_branch`, `origin/master` và `origin/duong_branch`.
...
#### 5.4 Cập nhật nâng cấp ByteTrack & Temporal Voting trong Notebook

Notebook `01.tsr_colab_production_lite_demo.ipynb` đã được tích hợp đồng bộ các cải tiến thực tế:

1. **Thuật toán phân bổ hai tầng ByteTrack** tại Cell `28`: tách detections thành tập điểm cao ($conf \ge 0.25$) và điểm thấp ($0.10 \le conf < 0.25$) để cứu các track bị mờ tạm thời do mưa hoặc chói sáng.
2. **Temporal Majority Voting** tại Cell `28`: cơ chế cửa sổ trượt 15 frames để bầu chọn nhãn và tốc độ giới hạn đa số, triệt tiêu hiện tượng nhấp nháy class.
3. **Producer-Consumer Threading** tại Cell `25`: bổ sung lớp `ThreadedVideoCapture` để tránh nghẽn I/O khi đọc video.
4. **Hỗ trợ ONNX Runtime** tại Cell `31`: tự động phát hiện và ưu tiên nạp model tối ưu `models/best.onnx` nếu có.
