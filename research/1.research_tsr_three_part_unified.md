...
```mermaid
flowchart TD
    A[12V Vehicle Power] --> B[Power Conditioning and Protection]
    B --> C[Front Camera]
    B --> D[ADAS ECU or Domain Controller]
    D --> E[RAM and Persistent Storage]
    D --> F[CAN FD or Automotive Ethernet]
    F --> G[Cluster or HUD]
    F --> H[Gateway]
    H --> I[GNSS and Navigation]
    H --> J[Diagnostics and OTA]
    D --> K[Thermal Monitor and Watchdog]
```

### I.6.2 Danh mục phần cứng phải chốt

| Hạng mục | Câu hỏi phải trả lời | Ví dụ |
|---|---|---|
| Camera trước | Độ phân giải, HDR/WDR, FOV, fps và shutter có đủ để biển còn đọc được trong ODD không | 2MP HDR 30fps đủ advisory đô thị; 60fps nếu camera còn share cho feature khác |
| Camera interface | Khoảng cách camera-ECU và băng thông yêu cầu là gì; có cần SerDes automotive không | Camera đặt xa ECU gần như luôn phải đi qua GMSL/FPD-Link |
| ECU | CPU/GPU/NPU, RAM, storage và nhiệt có đủ cho latency budget dài hạn không | Prototype có thể dùng Jetson; production nên là SoC automotive-grade |
| Storage | Lưu model/config ở đâu; có partition an toàn và rollback không | OTA model nên có A/B partition hoặc vùng rollback riêng |
| Vehicle network | Signal nào đi CAN FD, signal nào cần Ethernet; raw video có đi qua bus không | HMI chỉ cần signal nhỏ; raw video không đi qua CAN |
| HMI | Hiển thị ở cluster, HUD hay kênh âm thanh nào; policy cảnh báo thuộc ai | Cần map rõ icon, chime, voice với từng trạng thái feature |
| Power | Brownout, surge, reverse polarity ảnh hưởng gì tới camera và ECU | Brownout ngắn có thể làm camera restart và tạo output sai nếu không có health check |
| Mechanical/thermal | Mount, rung, bụi bẩn, nhiệt và tản nhiệt ảnh hưởng ổn định ra sao | Rung mạnh hoặc nóng kéo dài làm ảnh xấu và latency dao động |

**Ví dụ A: xe đô thị, advisory đơn giản**

- Camera 2MP HDR;
- 30fps;
- FOV vừa phải để biển không quá nhỏ;
- Ưu tiên dynamic range hơn độ phân giải quá cao.

**Ví dụ B: xe đi đường trường, muốn reuse camera cho nhiều tính năng**

- Cân bằng FOV rộng (lane/vehicle perception) và đủ chi tiết để TSR không mất biển nhỏ;
- Kiểm tra nhiệt, glare và phản xạ mạnh trên biển phản quang.
...
Mặc dù tên gọi khác nhau, phần lớn hệ thống production đều tuân theo nguyên tắc:
...
Một stack TSR production phải tách rõ **perception detection**, **sign object management**, **feature interpretation**, **HMI/ISA output**, và **diagnostics**. Detector chỉ trả lời câu hỏi "trong frame có vùng giống biển nào"; feature TSR trả lời câu hỏi "biển nào đang áp dụng cho xe ego, trong bao lâu, với mức tin cậy nào, và phải xuất ra đâu".
...
flowchart LR
    ...
    F --> G[Context Filter]
    G --> H[Camera Map Fusion]
    H --> I[TSR Feature State]
    I --> J[HMI Or ISA]
    I --> K[Diagnostics And Logging]
```
