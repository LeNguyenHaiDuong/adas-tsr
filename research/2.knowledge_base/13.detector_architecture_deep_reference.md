...
> **Thứ tự đọc:** đọc sau [Baseline Repo Analysis Full](../3.implementation/04.baseline_repo_analysis_full.md) `[SRC]` — Lấy: evidence baseline hiện tại, model/runtime và gap detector. Dùng ở: đặt các trade-off anchor/head/NMS vào đúng bối cảnh repo.
>
> **File này trả lời:** nếu cần đổi hoặc đánh giá detector cho TSR, nên soi vào những trade-off kiến trúc nào.
>
> **[SRC] Ngoài phạm vi:** kể lại production pipeline TSR từ đầu đến cuối; narrative đó nằm ở [Unified Production Reference](12.unified_production_reference.md) — Lấy: lifecycle, ODD, HMI và release context. Dùng ở: điều hướng khi detector trade-off cần nối sang feature/system behavior.
...
| Liên kết triển khai repo | [Baseline Repo Analysis Full](../3.implementation/04.baseline_repo_analysis_full.md) `[SRC]` |
| Liên kết narrative hệ thống | [Unified Production Reference](12.unified_production_reference.md) `[SRC]` |
...
> **Nguồn tại mục:** [Detector Architecture Overview](07.detector_architecture_overview.md) `[SRC]` — Lấy: detector là input cho feature TSR và các trade-off chính cần soi. Dùng ở: tóm tắt small-object, temporal stability và edge deployment.
>
...
- Khi cần trace biểu hiện của các trade-off này trong repo hiện tại, quay lại [Baseline Repo Analysis Full](../3.implementation/04.baseline_repo_analysis_full.md) — evidence implementation về baseline detector và gap runtime.
- Khi cần hệ quả system-level lên HMI, diagnostics hay maturity, quay lại [Unified Production Reference](12.unified_production_reference.md) — deep dive production cho feature TSR đầy đủ.
...
> **Nguồn tại mục:** [Detector Architecture Deep Reference](13.detector_architecture_deep_reference.md) `[SRC]` — Lấy: tổng hợp engineering trade-off head design áp dụng cho TSR từ detector literature và YOLO-family practice. Dùng ở: giải thích cls/reg conflict và ví dụ speed-sign digit.
>
...
> **Nguồn tại mục:** [Baseline Repo Analysis Full](../3.implementation/04.baseline_repo_analysis_full.md) `[SRC]` — Lấy: baseline hiện tại `Ultralytics YOLO11s`, CPU runtime và gap implementation. Dùng ở: dòng prototype CPU/current weights.
>
...
>
> **Nguồn tại mục:** [Baseline Repo Analysis Full](../3.implementation/04.baseline_repo_analysis_full.md) `[SRC]` — Lấy: `preprocess()`, `max_width`, `imgsz`, `non_max_suppression_simple()` và gap bbox-size KPI. Dùng ở: công thức resize, gap repo và implementation pattern.
>
...
> **Nguồn tại mục:** [Baseline Repo Analysis Full](../3.implementation/04.baseline_repo_analysis_full.md) `[SRC]` — Lấy: gap `max_width`, `imgsz`, NMS area-sort và thiếu size-bin KPI. Dùng ở: root cause các ví dụ small sign/miss/NMS.
>
...
>
> **Nguồn tại mục:** [Detector Architecture Deep Reference](13.detector_architecture_deep_reference.md) `[SRC]` — Lấy: tổng hợp pattern ROI pass, size-aware confidence và temporal accumulation từ design review. Dùng ở: bảng production implementation pattern.
>
> **Nguồn tại mục:** [Colab Production-Lite Demo Full](../3.implementation/05.colab_production_lite_demo_full.md) `[SRC]` — Lấy: temporal confirmation lite và event logging. Dùng ở: pattern temporal accumulation.
...
> **Nguồn tại mục:** [Baseline Repo Analysis Full](../3.implementation/04.baseline_repo_analysis_full.md) `[SRC]` — Lấy: runtime hiện tại resize theo `max_width`, dùng một `imgsz`, NMS sort theo diện tích và chưa có bin KPI. Dùng ở: bảng gap repo hiện tại.
...
>
> **Nguồn tại mục:** [Sources and Provenance](11.sources_and_provenance.md) `[SRC]` — Lấy: source-of-truth nội bộ cho dataset/model baseline. Dùng ở: phân biệt dataset công khai với evidence repo.
...
>
> **Nguồn tại mục:** [Experiment and Benchmark Guide](../3.implementation/03.experiment_benchmark_guide.md) `[SRC]` — Lấy: benchmark protocol, split hygiene và repeatability. Dùng ở: route-level split, scenario tags và benchmark report.
...
> **Nguồn tại mục:** [Baseline Repo Analysis Full](../3.implementation/04.baseline_repo_analysis_full.md) `[SRC]` — Lấy: baseline dataset/model hiện tại, long-tail và missing dataset artifacts. Dùng ở: gap dataset card, scenario matrix, split protocol và per-class/size KPI.
...
>
> **Nguồn tại mục:** [Baseline Repo Analysis Full](../3.implementation/04.baseline_repo_analysis_full.md) `[SRC]` — Lấy: repo hiện chưa có export/parity protocol. Dùng ở: gap/deployment steps.
...
>
> **Nguồn tại mục:** [Baseline Repo Analysis Full](../3.implementation/04.baseline_repo_analysis_full.md) `[SRC]` — Lấy: runtime gap hiện tại. Dùng ở: parity checker và benchmark protocol.
...
> **Nguồn tại mục:** [Baseline Repo Analysis Full](../3.implementation/04.baseline_repo_analysis_full.md) `[SRC]` — Lấy: repo hiện chỉ hướng dẫn PyTorch/Ultralytics inference, chưa có export/parity/calibration/runtime wrapper. Dùng ở: gap edge deployment.
>
...
>
> **Nguồn tại mục:** [Detector Architecture Deep Reference](13.detector_architecture_deep_reference.md) `[SRC]` — Lấy: synthesis về digit head/OCR/canonicalization từ detector trade-off và TSR feature needs. Dùng ở: production OCR pipeline pattern.
...
>
> **Nguồn tại mục:** [Baseline Repo Analysis Full](../3.implementation/04.baseline_repo_analysis_full.md) `[SRC]` — Lấy: nhánh traditional heuristic không đọc giá trị speed và runtime hiện không có OCR. Dùng ở: dòng `--traditional` heuristic.
...
> **Nguồn tại mục:** [Baseline Repo Analysis Full](../3.implementation/04.baseline_repo_analysis_full.md) `[SRC]` — Lấy: runtime hiện không có OCR/digit branch/canonicalization/plausibility check. Dùng ở: bảng gap repo.
...
>
> **Nguồn tại mục:** [Colab Production-Lite Demo Full](../3.implementation/05.colab_production_lite_demo_full.md) `[SRC]` — Lấy: confidence/quality gate, warning_level và replay KPI ở notebook. Dùng ở: engineering confidence cho production-lite pipeline.
...
> **Nguồn tại mục:** [State Manager and Sign Lifecycle](02.state_manager_and_sign_lifecycle.md) `[SRC]` — Lấy: hit/miss và temporal confirmation trước publish. Dùng ở: EMA/temporal confidence và `hits >= N`.
>
> **Nguồn tại mục:** [ODD for TSR](03.odd_for_tsr.md) `[SRC]` — Lấy: quality/ODD gate và degraded behavior. Dùng ở: `quality_score`, size-aware threshold và multi-dimensional publish rule.
>
...
> **Nguồn tại mục:** [Baseline Repo Analysis Full](../3.implementation/04.baseline_repo_analysis_full.md) `[SRC]` — Lấy: baseline dùng global confidence threshold, chưa có ECE/reliability/calibration artifact. Dùng ở: bảng gap repo.
>
> **Nguồn tại mục:** [Colab Production-Lite Demo Full](../3.implementation/05.colab_production_lite_demo_full.md) `[SRC]` — Lấy: notebook có quality/confidence gate lite nhưng chưa thành calibration bundle. Dùng ở: phân biệt production-lite confidence logic với production calibration.
...
>
> **Nguồn tại mục:** [Baseline Repo Analysis Full](../3.implementation/04.baseline_repo_analysis_full.md) `[SRC]` — Lấy: repo hiện chưa có feature-layer event KPI/export parity/ODD validation. Dùng ở: gap benchmark và yêu cầu lock protocol.
>
