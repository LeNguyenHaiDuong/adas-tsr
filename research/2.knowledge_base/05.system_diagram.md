...
> **Thứ tự đọc:** 5 — visual appendix sau [2.04. Unified Production Reference](04.unified_production_reference.md).
>
> **File này trả lời:** các block kiến trúc TSR nối với nhau như thế nào ở mức nhìn nhanh.
>
> **[SRC] Ngoài phạm vi:** giải thích dài về production mindset, gap baseline hay roadmap; các phần đó nằm ở [2.04. Unified Production Reference](04.unified_production_reference.md) — Lấy: narrative production đầy đủ. Dùng ở: điều hướng người đọc sang deep dive khi sơ đồ không đủ chi tiết.
>
> **Nguồn chính:**
>
...
> - [2.06. Sources and Provenance](06.sources_and_provenance.md) `[SRC]` — Lấy: quy ước `[SRC]` và danh mục evidence nội bộ. Dùng ở: chú thích nguồn cho sơ đồ và link sang provenance.

## Tổng quan kiến trúc

> **Nguồn tại mục:** [2.04. Unified Production Reference](04.unified_production_reference.md) `[SRC]` — Lấy: phân lớp production TSR thành perception, feature logic và vehicle integration. Dùng ở: ba lớp kiến trúc trong phần tổng quan.
>
...
> **Nguồn tại mục:** [2.04. Unified Production Reference](04.unified_production_reference.md) `[SRC]` — Lấy: stack detector → tracking → context/lane/fusion/state → HMI/ISA/diagnostics/logging. Dùng ở: sơ đồ Mermaid chính.
>
...
> **Nguồn tại mục:** [2.04. Unified Production Reference](04.unified_production_reference.md) `[SRC]` — Lấy: camera, map, vehicle dynamics và configuration là inputs cho applicability/fusion. Dùng ở: bảng input sources.
>
> **Nguồn tại mục:** [2.13. ISP Image Pipeline for TSR](13.isp_image_pipeline_for_tsr.md) `[SRC]` — Lấy: camera pipeline `CMOS -> RAW Bayer -> ISP -> RGB/YUV` trước detector. Dùng ở: block `Camera ISP`.

| Block | Vai trò ngắn | Xem chi tiết ở đâu |
|---|---|---|
| `Front Camera Module` | Nguồn ảnh chính cho perception | `research/1...` Phần I–II |
| `Camera ISP` | Biến RAW Bayer thành RGB/YUV qua AE, AWB, demosaic, denoise, HDR, tone mapping, lens correction và sharpening | [2.13. ISP Image Pipeline for TSR](13.isp_image_pipeline_for_tsr.md) |
...
>
> **Nguồn tại mục:** [3.12. Baseline Repo Analysis Full](../3.implementation/12.baseline_repo_analysis_full.md) `[SRC]` — Lấy: runtime chính có gì/chưa có gì. Dùng ở: cột `Repo hiện tại`.
...
>
> **Nguồn tại mục:** [2.08. State Manager and Sign Lifecycle](08.state_manager_and_sign_lifecycle.md) `[SRC]` — Lấy: lifecycle `CANDIDATE -> ACTIVE -> STALE -> EXPIRED`. Dùng ở: `TSR State Manager`.
...
>
> **Nguồn tại mục:** [3.11. Colab Production-Lite Demo Full](../3.implementation/11.colab_production_lite_demo_full.md) `[SRC]` — Lấy: `events.jsonl` replay/logging evidence. Dùng ở: `Data Logging`.
...
> **Nguồn tại mục:** [2.04. Unified Production Reference](04.unified_production_reference.md) `[SRC]` — Lấy: signal flow từ camera/detection/tracking/fusion/state đến HMI/ISA. Dùng ở: sequence diagram.
...
>
> **Nguồn tại mục:** [2.08. State Manager and Sign Lifecycle](08.state_manager_and_sign_lifecycle.md) `[SRC]` — Lấy: tracking + confirmation + active state. Dùng ở: các bước tracking, state manager và HMI/ISA publish.
...
| Nếu bạn cần... | Hãy mở |
|---|---|
| Câu chuyện hệ thống đầy đủ và roadmap production | [2.04. Unified Production Reference](04.unified_production_reference.md) `[SRC]` |
| Gap implementation của repo hiện tại | [3.12. Baseline Repo Analysis Full](../3.implementation/12.baseline_repo_analysis_full.md) `[SRC]` |
| Trade-off detector, small-object, edge deploy | [3.10. Detector Architecture Deep Reference](../3.implementation/10.detector_architecture_deep_reference.md) `[SRC]` |
| Demo stateful replay trên Colab | [3.11. Colab Production-Lite Demo Full](../3.implementation/11.colab_production_lite_demo_full.md) `[SRC]` |
