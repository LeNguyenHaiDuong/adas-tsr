...
- xác định mức rủi ro MSIL từ A đến D cho xe hai bánh, với scope prototype nên giả định tối thiểu `MSIL B` và tăng lên `MSIL C` nếu cảnh báo tốc độ được dùng như đầu vào ISA/HMI có tác động mạnh;
- với rủi ro nghiêm trọng, dùng quy trình nghiêm ngặt tương đương ánh xạ MSIL D sang tư duy ASIL C của ô tô;
- dùng Hội đồng CCP (Controllability Classification Panel) để đánh giá khả năng kiểm soát của người lái trong các tình huống xe máy rung lắc, vào cua, đường trơn hoặc cảnh báo tốc độ đến muộn;
- kiểm soát lỗi phần cứng, lỗi phần mềm, lỗi giao tiếp camera/ECU/HMI và trạng thái degraded/unavailable.

## 2.1 MSIL Và CCP Cho Xe Máy

Xe máy có động học khác ô tô: vào cua nghiêng thân xe, mặt đường xấu gây rung mạnh, khoảng thời gian người lái phản ứng ngắn và không có cabin bảo vệ. Vì vậy, TSR advisory tưởng như "chỉ hiển thị" vẫn cần phân tích controllability.

| Mức đề xuất | Khi áp dụng cho TSR | Kỳ vọng quy trình |
|---|---|---|
| `MSIL B` | TSR chỉ là advisory, không can thiệp điều khiển, HMI có degraded/unavailable rõ. | Có hazard analysis, fault detection, test ODD/weather và evidence runtime. |
| `MSIL C` | TSR được dùng cho cảnh báo tốc độ có tính ràng buộc mạnh hoặc feed sang ISA. | Cần V&V chặt hơn, traceability, fault injection và CCP review. |
| `MSIL D` | TSR ảnh hưởng trực tiếp đến điều khiển hoặc feature tốc độ tự động. | Không thuộc scope repo; nếu mở rộng phải nâng lên quy trình tương đương ASIL C/D tùy item. |

CCP nên xem ít nhất các tình huống:

- người lái đang vào cua và HMI đột ngột clear biển tốc độ;
- cảnh báo tốc độ đến muộn sau khi qua biển;
- camera degraded do mưa nhưng HMI vẫn hiển thị như bình thường;
- rung động làm cáp/camera chập chờn, gây stale data.
...
- cần nhận diện biển giới hạn tốc độ cục bộ và biển phụ (sub-signs) vì chúng quyết định phạm vi áp dụng;
...
<!-- consolidated-from: research/2.knowledge_base/00.index.md -->

## Phụ Lục Hợp Nhất Từ `research/2.knowledge_base/00.index.md`

> Phần này được gộp từ `research/2.knowledge_base/00.index.md` để giữ lại nội dung và traceability sau khi rút gọn cấu trúc Markdown.
...
Phần index cũ của Knowledge Base đã được hợp nhất vào file hiện tại. Prompt v3 chuẩn hóa tầng này thành nơi tiếp nhận gap từ narrative và chuyển hóa thành tiêu chuẩn/đặc tả thiết kế, không kể lại code hoặc hướng dẫn chạy lệnh.
...
| Nhóm | File | Vai trò |
|---|---|---|
| Production reference | [Unified Production Reference](01.automotive_standards.md) | Production TSR end-to-end: feature architecture, ODD, SOTIF, HMI, diagnostics, V&V và roadmap |
...
| Legacy v1 camera safety | [Camera HAZOP, SOTIF, and FTA](03.safety_analysis_hazop_fta.md) | Bản refactor v1 được giữ để không mất nội dung |
| Legacy v1 camera spec | [Camera Spec and IEEE 2020](02.camera_hardware_ieee2020.md) | Bản refactor v1 được giữ để không mất nội dung |
...
| `Production` | Feature TSR cấp xe, không đồng nghĩa với detector | [2.04. Unified Production Reference](01.automotive_standards.md) `[SRC]` và [ISO 26262-1:2018](https://www.iso.org/standard/68383.html) — Lấy: lifecycle/release evidence. Dùng ở: định nghĩa maturity, release gate và scope feature cấp xe. |
...
- File hiện tại giữ vai trò điều hướng và tiêu chuẩn cốt lõi của Knowledge Base sau khi hợp nhất.
...
- Luồng chính mới là [2.01](01.automotive_standards.md) -> [2.02](02.camera_hardware_ieee2020.md) -> [2.03](03.safety_analysis_hazop_fta.md).
- Các short note `2.07` đến `2.12` giữ vai trò checkpoint theo chủ đề; không thay thế bản production dài ở [Unified Production Reference](01.automotive_standards.md).
...
<!-- consolidated-from: research/2.knowledge_base/04.unified_production_reference.md -->

## Phụ Lục Hợp Nhất Từ `research/2.knowledge_base/04.unified_production_reference.md`

> Phần này được gộp từ `research/2.knowledge_base/04.unified_production_reference.md` để giữ lại nội dung và traceability sau khi rút gọn cấu trúc Markdown.
...
>
> **File này trả lời:** production feature TSR cần những block nào, baseline demo còn thiếu gì, và roadmap repo nên đi theo chuỗi nào.
>
...
| Tài liệu chuyên đề | [3.12. Baseline Repo Analysis Full](../3.implementation/01.hybrid_pipeline_architecture.md) `[SRC]`, [3.10. Detector Architecture Deep Reference](../3.implementation/01.hybrid_pipeline_architecture.md) `[SRC]`, [3.11. Colab Production-Lite Demo Full](../3.implementation/03.edge_deployment_and_benchmarks.md) `[SRC]` |
| Tài liệu dẫn xuất | [2.05. System Diagram](01.automotive_standards.md) `[SRC]` |
| Sơ đồ HTML Phần II | Đã loại khỏi repo khi bỏ thư mục asset tĩnh cũ; giữ mô tả Mermaid trong tài liệu này làm nguồn đọc chính. |
...
> **Nguồn tại mục:** [AUTOSAR Classic Platform](https://www.autosar.org/standards/classic-platform), [ISO 26262-1:2018](https://www.iso.org/standard/68383.html) và [3.12. Baseline Repo Analysis Full](../3.implementation/01.hybrid_pipeline_architecture.md) `[SRC]` — Lấy: phân lớp SWC/platform, fault containment và gap monolithic script. Dùng ở: task-based architecture, health monitor, degraded operation và timing contract.
...
File HTML tương tác cũ đã được loại khỏi repo theo phạm vi hiện tại. Tài liệu này giữ lại mô tả kiến trúc bằng bảng và Mermaid để không phụ thuộc vào asset ngoài luồng tuyến tính.
...
> **Nguồn tại mục:** [ISO 26262-1:2018](https://www.iso.org/standard/68383.html), [ISO 21448:2022](https://www.iso.org/standard/77490.html), [Automotive SPICE / VDA QMC](https://vda-qmc.de/en/automotive-spice/) và [3.12. Baseline Repo Analysis Full](../3.implementation/01.hybrid_pipeline_architecture.md) `[SRC]` — Lấy: verification lifecycle, scenario validation, process evidence và KPI/release gates. Dùng ở: V-model, test gates, KPI production và embedded release checklist.

> **KPI gates, release flow và failure RCA chi tiết:** [3.12. Baseline Repo Analysis Full](../3.implementation/01.hybrid_pipeline_architecture.md) — Lấy: evidence implementation cho KPI gates, release flow và failure RCA; xem §10–§14. Dùng ở: phần release/checklist của production reference, không lặp bảng KPI gate §10.1 hoặc release flow §11.
...
> **Nguồn tại mục:** [2.10. SOTIF for TSR](01.automotive_standards.md) `[SRC]`, [2.09. ODD for TSR](01.automotive_standards.md) `[SRC]` và [3.12. Baseline Repo Analysis Full](../3.implementation/01.hybrid_pipeline_architecture.md) `[SRC]` — Lấy: trigger conditions, ODD gap và baseline failure modes. Dùng ở: case demo chưa cover cho perception/system/HMI.
...
> **Nguồn tại mục:** [3.12. Baseline Repo Analysis Full](../3.implementation/01.hybrid_pipeline_architecture.md) `[SRC]`, [3.11. Colab Production-Lite Demo Full](../3.implementation/03.edge_deployment_and_benchmarks.md) `[SRC]` và [3.08. Experiment and Benchmark Guide](../3.implementation/03.edge_deployment_and_benchmarks.md) `[SRC]` — Lấy: gap theo code, production-lite block và benchmark/replay sequencing. Dùng ở: thứ tự triển khai JSONL → lifecycle → diagnostics → parity/stub.
...
| Gap chi tiết theo code: [3.12. Baseline Repo Analysis Full](../3.implementation/01.hybrid_pipeline_architecture.md) | Ma trận 14 gap → triệu chứng cụ thể |
...
| KPI and validation metrics used by OEMs | II.10 (tổng quan), [3.12. Baseline Repo Analysis Full](../3.implementation/01.hybrid_pipeline_architecture.md) §10–§12 |
| Failure Mode and Root Cause Analysis | I.9 (malfunction), [3.12. Baseline Repo Analysis Full](../3.implementation/01.hybrid_pipeline_architecture.md) §13–§14 |
...
> **Nguồn tại mục:** [3.10. Detector Architecture Deep Reference](../3.implementation/01.hybrid_pipeline_architecture.md) `[SRC]`, [3.12. Baseline Repo Analysis Full](../3.implementation/01.hybrid_pipeline_architecture.md) `[SRC]` và [2.06. Sources and Provenance](01.automotive_standards.md) `[SRC]` — Lấy: feedback review, dataset/source caveat và KPI/release evidence. Dùng ở: summary cách tài liệu đã xử lý phản biện.
...
| **Methodologist** | KPI phải có release gate, không chỉ liệt kê metric | [3.12. Baseline Repo Analysis Full](../3.implementation/01.hybrid_pipeline_architecture.md) §10–§11: gate flow và SLIF ≥90% |
...
- Ngưỡng KPI "gợi ý" trong [3.12. Baseline Repo Analysis Full](../3.implementation/01.hybrid_pipeline_architecture.md) §10.1 là pattern OEM thường dùng, **không** phải số contract cố định — mỗi OEM negotiate riêng.
...
- **(1) Tài liệu production mở rộng (file này):** [2.04. Unified Production Reference](01.automotive_standards.md)
- Sơ đồ ECU HTML cũ đã được loại khỏi repo cùng thư mục asset tĩnh cũ; dùng phần II.3 trong tài liệu này làm reference thay thế.
- **(2) Baseline gắn code:** [3.12. Baseline Repo Analysis Full](../3.implementation/01.hybrid_pipeline_architecture.md)
...
<!-- consolidated-from: research/2.knowledge_base/05.system_diagram.md -->

## Phụ Lục Hợp Nhất Từ `research/2.knowledge_base/05.system_diagram.md`

> Phần này được gộp từ `research/2.knowledge_base/05.system_diagram.md` để giữ lại nội dung và traceability sau khi rút gọn cấu trúc Markdown.

## TSR System Architecture

> **Thứ tự đọc:** 5 — visual appendix sau [2.04. Unified Production Reference](01.automotive_standards.md).
>
> **File này trả lời:** các block kiến trúc TSR nối với nhau như thế nào ở mức nhìn nhanh.
>
> **[SRC] Ngoài phạm vi:** giải thích dài về production mindset, gap baseline hay roadmap; các phần đó nằm ở [2.04. Unified Production Reference](01.automotive_standards.md) — Lấy: narrative production đầy đủ. Dùng ở: điều hướng người đọc sang deep dive khi sơ đồ không đủ chi tiết.
>
> **Nguồn chính:**
>
...
> **Nguồn tại mục:** [2.04. Unified Production Reference](01.automotive_standards.md) `[SRC]` — Lấy: phân lớp production TSR thành perception, feature logic và vehicle integration. Dùng ở: ba lớp kiến trúc trong phần tổng quan.
>
...
> **Nguồn tại mục:** [2.04. Unified Production Reference](01.automotive_standards.md) `[SRC]` — Lấy: stack detector → tracking → context/lane/fusion/state → HMI/ISA/diagnostics/logging. Dùng ở: sơ đồ Mermaid chính.
>
...
> **Nguồn tại mục:** [2.04. Unified Production Reference](01.automotive_standards.md) `[SRC]` — Lấy: camera, map, vehicle dynamics và configuration là inputs cho applicability/fusion. Dùng ở: bảng input sources.
>
...
>
> **Nguồn tại mục:** [3.12. Baseline Repo Analysis Full](../3.implementation/01.hybrid_pipeline_architecture.md) `[SRC]` — Lấy: runtime chính có gì/chưa có gì. Dùng ở: cột `Repo hiện tại`.
...
> **Nguồn tại mục:** [2.04. Unified Production Reference](01.automotive_standards.md) `[SRC]` — Lấy: signal flow từ camera/detection/tracking/fusion/state đến HMI/ISA. Dùng ở: sequence diagram.
...
| Nếu bạn cần... | Hãy mở |
|---|---|
| Câu chuyện hệ thống đầy đủ và roadmap production | [2.04. Unified Production Reference](01.automotive_standards.md) `[SRC]` |
| Gap implementation của repo hiện tại | [3.12. Baseline Repo Analysis Full](../3.implementation/01.hybrid_pipeline_architecture.md) `[SRC]` |
...
<!-- consolidated-from: research/2.knowledge_base/06.sources_and_provenance.md -->

## Phụ Lục Hợp Nhất Từ `research/2.knowledge_base/06.sources_and_provenance.md`

> Phần này được gộp từ `research/2.knowledge_base/06.sources_and_provenance.md` để giữ lại nội dung và traceability sau khi rút gọn cấu trúc Markdown.
...
- `[SRC]` Tài liệu production mở rộng: [2.04. Unified Production Reference](01.automotive_standards.md) — Lấy: production stack, safety/SOTIF/HMI/diagnostics/release roadmap. Dùng ở: source chuẩn nội bộ cho kiến trúc feature TSR.
...
- `[SRC]` Baseline gắn code: [3.12. Baseline Repo Analysis Full](../3.implementation/01.hybrid_pipeline_architecture.md) — Lấy: phân tích chi tiết code, runtime, checkpoint và gap. Dùng ở: các mục “Relation to Current Prototype”.
...
- Các nguồn này được dùng để củng cố phần ODD, driving modes, hệ thống state transition, và verification logic trong [2.04. Unified Production Reference](01.automotive_standards.md) `[SRC]`.
...
- Các nguồn trong mục này hỗ trợ [3.10. Detector Architecture Deep Reference](../3.implementation/01.hybrid_pipeline_architecture.md) `[SRC]` cho small-object, edge, dataset và [2.04. Unified Production Reference](01.automotive_standards.md) `[SRC]` cho ISA, SOTIF, fusion.
...
- Council Review Summary nằm ở [2.04. Unified Production Reference](01.automotive_standards.md) `[SRC]` Phụ lục C.


---

<!-- consolidated-from: research/2.knowledge_base/07.tsr_feature_architecture.md -->

## Phụ Lục Hợp Nhất Từ `research/2.knowledge_base/07.tsr_feature_architecture.md`

> Phần này được gộp từ `research/2.knowledge_base/07.tsr_feature_architecture.md` để giữ lại nội dung và traceability sau khi rút gọn cấu trúc Markdown.
...
> **[SRC] Bài đọc sâu hơn:**
>
> - [2.04. Unified Production Reference](01.automotive_standards.md) — Lấy: danh sách block production, roadmap và trade-off hệ thống. Dùng ở: mục kiến trúc 3 lớp perception/feature logic/vehicle interface.
...
> **Nguồn tại mục:** [2.04. Unified Production Reference](01.automotive_standards.md) `[SRC]` — Lấy: phân tách perception, feature logic và vehicle integration. Dùng ở: định nghĩa feature TSR không dừng ở detector output.
>
...
> **Nguồn tại mục:** [2.04. Unified Production Reference](01.automotive_standards.md) `[SRC]` — Lấy: pattern ba lớp `Perception`, `Feature Logic`, `Vehicle Integration`. Dùng ở: danh sách lớp production TSR.
>
...
<!-- consolidated-from: research/2.knowledge_base/09.odd_for_tsr.md -->

## Phụ Lục Hợp Nhất Từ `research/2.knowledge_base/09.odd_for_tsr.md`

> Phần này được gộp từ `research/2.knowledge_base/09.odd_for_tsr.md` để giữ lại nội dung và traceability sau khi rút gọn cấu trúc Markdown.
...
> **[SRC] Full reference:**
>
> - [2.04. Unified Production Reference](01.automotive_standards.md) — Lấy: ODD taxonomy, degraded behavior và release gate theo ODD slice. Dùng ở: thiết kế ODD gate và behavior khi input ngoài phạm vi.
...
<!-- consolidated-from: research/2.knowledge_base/10.sotif_for_tsr.md -->

## Phụ Lục Hợp Nhất Từ `research/2.knowledge_base/10.sotif_for_tsr.md`

> Phần này được gộp từ `research/2.knowledge_base/10.sotif_for_tsr.md` để giữ lại nội dung và traceability sau khi rút gọn cấu trúc Markdown.
...
>
> **[SRC] Full reference:** [2.04. Unified Production Reference](01.automotive_standards.md) — Lấy: SOTIF trigger catalog, ODD monitor và release evidence. Dùng ở: phần residual risk, degraded behavior và checklist artifact production.
>
> **Nguồn chính:**
>
...
>
> **Nguồn tại mục:** [2.04. Unified Production Reference](01.automotive_standards.md) `[SRC]` — Lấy: production pattern temporal confirmation, suppression, degraded behavior và context validation. Dùng ở: mitigation examples.
...
<!-- consolidated-from: research/2.knowledge_base/11.verification_validation_and_release.md -->

## Phụ Lục Hợp Nhất Từ `research/2.knowledge_base/11.verification_validation_and_release.md`

> Phần này được gộp từ `research/2.knowledge_base/11.verification_validation_and_release.md` để giữ lại nội dung và traceability sau khi rút gọn cấu trúc Markdown.
...
> **[SRC] Full reference:**
>
> - [2.04. Unified Production Reference](01.automotive_standards.md) — Lấy: V-model, work products, release gates và roadmap production. Dùng ở: checklist verification/validation/release của feature TSR.
> - [3.12. Baseline Repo Analysis Full](../3.implementation/01.hybrid_pipeline_architecture.md) — Lấy: evidence baseline runtime, missing tests, missing metrics và release gaps. Dùng ở: phần “repo hiện tại đã/chưa có gì”.
>
> **Nguồn chính:**
>
...
> **Nguồn tại mục:** [3.12. Baseline Repo Analysis Full](../3.implementation/01.hybrid_pipeline_architecture.md) `[SRC]` — Lấy: baseline hiện mới có demo/runtime characterization, thiếu release discipline. Dùng ở: cảnh báo không đánh giá bằng video đẹp/model card.
>
...
>
> **Nguồn tại mục:** [2.04. Unified Production Reference](01.automotive_standards.md) `[SRC]` — Lấy: mapping release maturity từ offline demo đến pilot/vehicle validation. Dùng ở: bảng test levels cho TSR.
...
>
> **Nguồn tại mục:** [3.12. Baseline Repo Analysis Full](../3.implementation/01.hybrid_pipeline_architecture.md) `[SRC]` — Lấy: KPI/release gap hiện tại. Dùng ở: pattern benchmark/replay cần chuẩn hóa.
