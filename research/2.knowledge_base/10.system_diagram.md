...
>
> **[SRC] Ngoài phạm vi:** giải thích dài về production mindset, gap baseline hay roadmap; các phần đó nằm ở [Unified Production Reference](12.unified_production_reference.md) — Lấy: narrative production đầy đủ. Dùng ở: điều hướng người đọc sang deep dive khi sơ đồ không đủ chi tiết.
>
> **Nguồn chính:**
>
...
> - [Sources and Provenance](11.sources_and_provenance.md) `[SRC]` — Lấy: quy ước `[SRC]` và danh mục evidence nội bộ. Dùng ở: chú thích nguồn cho sơ đồ và link sang provenance.

## Tổng quan kiến trúc

> **Nguồn tại mục:** [Unified Production Reference](12.unified_production_reference.md) `[SRC]` — Lấy: phân lớp production TSR thành perception, feature logic và vehicle integration. Dùng ở: ba lớp kiến trúc trong phần tổng quan.
>
...
> **Nguồn tại mục:** [Unified Production Reference](12.unified_production_reference.md) `[SRC]` — Lấy: stack detector → tracking → context/lane/fusion/state → HMI/ISA/diagnostics/logging. Dùng ở: sơ đồ Mermaid chính.
>
...
> **Nguồn tại mục:** [Unified Production Reference](12.unified_production_reference.md) `[SRC]` — Lấy: camera, map, vehicle dynamics và configuration là inputs cho applicability/fusion. Dùng ở: bảng input sources.
...
>
> **Nguồn tại mục:** [Baseline Repo Analysis Full](../3.implementation/04.baseline_repo_analysis_full.md) `[SRC]` — Lấy: runtime chính có gì/chưa có gì. Dùng ở: cột `Repo hiện tại`.
...
>
> **Nguồn tại mục:** [State Manager and Sign Lifecycle](02.state_manager_and_sign_lifecycle.md) `[SRC]` — Lấy: lifecycle `CANDIDATE -> ACTIVE -> STALE -> EXPIRED`. Dùng ở: `TSR State Manager`.
...
>
> **Nguồn tại mục:** [Colab Production-Lite Demo Full](../3.implementation/05.colab_production_lite_demo_full.md) `[SRC]` — Lấy: `events.jsonl` replay/logging evidence. Dùng ở: `Data Logging`.
...
> **Nguồn tại mục:** [Unified Production Reference](12.unified_production_reference.md) `[SRC]` — Lấy: signal flow từ camera/detection/tracking/fusion/state đến HMI/ISA. Dùng ở: sequence diagram.
...
>
> **Nguồn tại mục:** [State Manager and Sign Lifecycle](02.state_manager_and_sign_lifecycle.md) `[SRC]` — Lấy: tracking + confirmation + active state. Dùng ở: các bước tracking, state manager và HMI/ISA publish.
...
| Nếu bạn cần... | Hãy mở |
|---|---|
| Câu chuyện hệ thống đầy đủ và roadmap production | [Unified Production Reference](12.unified_production_reference.md) `[SRC]` |
| Gap implementation của repo hiện tại | [Baseline Repo Analysis Full](../3.implementation/04.baseline_repo_analysis_full.md) `[SRC]` |
| Trade-off detector, small-object, edge deploy | [Detector Architecture Deep Reference](13.detector_architecture_deep_reference.md) `[SRC]` |
| Demo stateful replay trên Colab | [Colab Production-Lite Demo Full](../3.implementation/05.colab_production_lite_demo_full.md) `[SRC]` |
