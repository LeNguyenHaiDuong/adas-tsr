...
| Tài liệu chuyên đề | [3.12. Baseline Repo Analysis Full](../3.implementation/12.baseline_repo_analysis_full.md) `[SRC]`, [3.10. Detector Architecture Deep Reference](../3.implementation/10.detector_architecture_deep_reference.md) `[SRC]`, [3.11. Colab Production-Lite Demo Full](../3.implementation/11.colab_production_lite_demo_full.md) `[SRC]` |
| Tài liệu dẫn xuất | [2.05. System Diagram](05.system_diagram.md) `[SRC]` |
...
| Nguồn tham khảo | [2.06. Sources and Provenance](06.sources_and_provenance.md) `[SRC]` |
...
| Folder | Index |
|---|---|
| Knowledge Base | [2.00. Knowledge Base Index](00.index.md) |
| Implementation | [3.00. Implementation Index](../3.implementation/00.index.md) |

File này chỉ giữ phần production deep reference.

### 0.2 Nguồn chuẩn và link nhanh

Các phần safety, SOTIF, HMI, diagnostics, cybersecurity, OTA và ISA trong tài liệu này dùng các nguồn chính dưới đây. Khi viết báo cáo hoặc slide, ưu tiên link này và đối chiếu thêm [2.06. Sources and Provenance](06.sources_and_provenance.md) `[SRC]`.
...
> **Nguồn tại mục:** [2.04. Unified Production Reference](04.unified_production_reference.md) `[SRC]`, [2.08. State Manager and Sign Lifecycle](08.state_manager_and_sign_lifecycle.md) `[SRC]` và [ISO 21448:2022](https://www.iso.org/standard/77490.html) — Lấy: phân lớp perception → temporal/context → feature output và ODD/SOTIF gate. Dùng ở: bảng năm lớp xử lý trước khi publish HMI/ISA.
...
> **Nguồn tại mục:** [ISO 26262 package overview](https://www.iso.org/publication/PUB200262.html), [ISO 21448:2022](https://www.iso.org/standard/77490.html) và [2.09. ODD for TSR](09.odd_for_tsr.md) `[SRC]` — Lấy: item definition, authority, ODD và fallback/degraded behavior. Dùng ở: item definition phase-1 cho TSR advisory-only.
...
> **Nguồn tại mục:** [ISO 26262-1:2018](https://www.iso.org/standard/68383.html), [Automotive SPICE / VDA QMC](https://vda-qmc.de/en/automotive-spice/) và [2.08. Verification Validation and Release](11.verification_validation_and_release.md) `[SRC]` — Lấy: lifecycle evidence, traceability và release gate mindset. Dùng ở: chuỗi work products item definition → verification → release readiness.
...
> **Nguồn tại mục:** [2.05. System Diagram](05.system_diagram.md) `[SRC]`, [AUTOSAR Classic Platform](https://www.autosar.org/standards/classic-platform) và [ISO 14229-1:2026](https://www.iso.org/standard/87962.html) — Lấy: block decomposition, ECU/software boundary và diagnostics interface. Dùng ở: kiến trúc camera/ECU/perception/state/HMI/gateway/service.
...
> **Nguồn tại mục:** [ISO 16750-1:2023](https://www.iso.org/standard/77578.html), [AUTOSAR Classic Platform](https://www.autosar.org/standards/classic-platform) và [3.10. Detector Architecture Deep Reference](../3.implementation/10.detector_architecture_deep_reference.md) `[SRC]` — Lấy: vehicle environmental robustness, platform boundary và edge/runtime constraints. Dùng ở: camera, ECU, storage, network, power, thermal và hardware trade-off của TSR.
...
> **Nguồn tại mục:** [AUTOSAR Classic Platform](https://www.autosar.org/standards/classic-platform), [ISO 26262-1:2018](https://www.iso.org/standard/68383.html) và [3.12. Baseline Repo Analysis Full](../3.implementation/12.baseline_repo_analysis_full.md) `[SRC]` — Lấy: phân lớp SWC/platform, fault containment và gap monolithic script. Dùng ở: task-based architecture, health monitor, degraded operation và timing contract.
...
> **Nguồn tại mục:** [ISO 21448:2022](https://www.iso.org/standard/77490.html), [Setting AI in context](https://arxiv.org/abs/2201.11451), [IEEE Std 2020-2024 / P2020](https://standards.ieee.org/ieee/2020/11960/) và [2.09. ODD for TSR](09.odd_for_tsr.md) `[SRC]` — Lấy: ODD boundary, trigger conditions, automotive image-quality attributes và state/degraded behavior. Dùng ở: phase-1 ODD, quality gate, out-of-scope ODD và state flow `INIT` → `SAFE_DISABLE`.
...
> **Nguồn tại mục:** [2.00. Knowledge Base Index — Glossary nhanh](00.index.md#glossary-nhanh) `[SRC]` và [2.06. Sources and Provenance](06.sources_and_provenance.md) `[SRC]` — Lấy: thuật ngữ chuẩn nội bộ và provenance của từng nhóm nguồn. Dùng ở: bảng thuật ngữ nền, common mistakes và cách đọc khái niệm trong toàn bộ research.
...
> **Nguồn tại mục:** [3.12. Baseline Repo Analysis Full](../3.implementation/12.baseline_repo_analysis_full.md) `[SRC]`, [1.01. Prototype to Production Narrative](../1.narrative/01.prototype_to_production.md) `[SRC]` và [2.05. System Diagram](05.system_diagram.md) `[SRC]` — Lấy: gap detector frame-level, block production cần thêm và kiến trúc feature. Dùng ở: sáu gap baseline và stack `Perception/Feature/Vehicle Integration`.
...
> **Nguồn tại mục:** [2.05. System Diagram](05.system_diagram.md) `[SRC]`, [2.08. State Manager and Sign Lifecycle](08.state_manager_and_sign_lifecycle.md) `[SRC]`, [HMI Diagnostics and Vehicle Interface](12.hmi_diagnostics_and_vehicle_interface.md) `[SRC]`, [IEEE Std 2020-2024 / P2020](https://standards.ieee.org/ieee/2020/11960/) và [AUTOSAR Classic Platform](https://www.autosar.org/standards/classic-platform) — Lấy: pipeline camera → quality gate → detector → state/context → HMI/diagnostics, image-quality attributes và ECU layering. Dùng ở: sơ đồ end-to-end, quality gate và phân rã block theo trách nhiệm.
...
> **Nguồn tại mục:** [ISO 21448:2022](https://www.iso.org/standard/77490.html), [MapFusion](https://arxiv.org/abs/2103.05929), [2.09. ODD for TSR](09.odd_for_tsr.md) `[SRC]` và [HMI Diagnostics and Vehicle Interface](12.hmi_diagnostics_and_vehicle_interface.md) `[SRC]` — Lấy: ODD/context gate, camera-map fusion concept và publish policy. Dùng ở: ba câu hỏi lane/context/fusion trước khi publish sign.
...
> **Nguồn tại mục:** [ISO 26262-1:2018](https://www.iso.org/standard/68383.html), [ISO 21448:2022](https://www.iso.org/standard/77490.html), [Automotive SPICE / VDA QMC](https://vda-qmc.de/en/automotive-spice/) và [3.12. Baseline Repo Analysis Full](../3.implementation/12.baseline_repo_analysis_full.md) `[SRC]` — Lấy: verification lifecycle, scenario validation, process evidence và KPI/release gates. Dùng ở: V-model, test gates, KPI production và embedded release checklist.

> **KPI gates, release flow và failure RCA chi tiết:** [3.12. Baseline Repo Analysis Full](../3.implementation/12.baseline_repo_analysis_full.md) — Lấy: evidence implementation cho KPI gates, release flow và failure RCA; xem §10–§14. Dùng ở: phần release/checklist của production reference, không lặp bảng KPI gate §10.1 hoặc release flow §11.
...
> **Nguồn tại mục:** [ISO 26262-1:2018](https://www.iso.org/standard/68383.html), [ISO 21448:2022](https://www.iso.org/standard/77490.html), [UNECE R156](https://unece.org/transport/documents/2021/03/standards/un-regulation-no-156-software-update-and-software-update) và [2.08. Verification Validation and Release](11.verification_validation_and_release.md) `[SRC]` — Lấy: evidence package, validation work products và update/rollback concept. Dùng ở: checklist trước pilot.
...
> **Nguồn tại mục:** [Requirements Engineering for Automotive Perception Systems](https://arxiv.org/abs/2302.12155), [2.09. ODD for TSR](09.odd_for_tsr.md) `[SRC]` và [2.05. System Diagram](05.system_diagram.md) `[SRC]` — Lấy: perception requirements theo context/operating condition và lane/context block trong TSR. Dùng ở: ego-lane applicability, topology pending và camera-only stub boundary.
...
> **Nguồn tại mục:** [ISO 21448:2022](https://www.iso.org/standard/77490.html), [2.09. ODD for TSR](09.odd_for_tsr.md) `[SRC]` và [2.10. SOTIF for TSR](10.sotif_for_tsr.md) `[SRC]` — Lấy: trigger condition, ODD/quality gate và residual-risk mitigation. Dùng ở: reason-coded suppression, applicability confidence và degraded/unavailable behavior.
...
> **Nguồn tại mục:** [ISO 21448:2022](https://www.iso.org/standard/77490.html), [IEEE Std 2020-2024 / P2020](https://standards.ieee.org/ieee/2020/11960/), [Organization of ML-based product development under ISO 26262/SOTIF](https://arxiv.org/abs/1910.05112), [GLARE Traffic Sign Dataset](https://arxiv.org/abs/2209.08716) và [2.10. SOTIF for TSR](10.sotif_for_tsr.md) `[SRC]` — Lấy: SOTIF trigger condition, automotive image-quality attributes, ML evidence lifecycle và glare dataset example. Dùng ở: trigger catalog, quality gate, mitigation pattern và residual risk argument.
...
> **Nguồn tại mục:** [ISO 26262-1:2018](https://www.iso.org/standard/68383.html), [ISO 21448:2022](https://www.iso.org/standard/77490.html), [UNECE R156](https://unece.org/transport/documents/2021/03/standards/un-regulation-no-156-software-update-and-software-update) và [2.08. Verification Validation and Release](11.verification_validation_and_release.md) `[SRC]` — Lấy: safety lifecycle evidence, SOTIF residual risk và post-release/update governance. Dùng ở: safety case package, authority-level evidence và residual risk review.
...
> **Nguồn tại mục:** [3.12. Baseline Repo Analysis Full](../3.implementation/12.baseline_repo_analysis_full.md) `[SRC]`, [3.05. Colab Production-Lite Demo](../3.implementation/05.colab_production_lite_demo.md) `[SRC]` và [3.06. Notebook Analyze](../3.implementation/06.notebook_analyze.md) `[SRC]` — Lấy: số đo baseline lịch sử, video replay hiện có `59.94 fps`, và lịch `skip` của notebook `3.NB01`. Dùng ở: tách rõ baseline benchmark, video FPS, scheduled inference rate và measured processing FPS.
...
> **Nguồn tại mục:** [3.12. Baseline Repo Analysis Full](../3.implementation/12.baseline_repo_analysis_full.md) `[SRC]`, [Ultralytics Benchmark Mode](https://docs.ultralytics.com/modes/benchmark/) và [3.10. Detector Architecture Deep Reference](../3.implementation/10.detector_architecture_deep_reference.md) `[SRC]` — Lấy: runtime local, benchmark discipline và edge-runtime constraints. Dùng ở: RAM/FPS/latency budget theo state và degraded mode.
...
> **Nguồn tại mục:** [3.11. Colab Production-Lite Demo Full](../3.implementation/11.colab_production_lite_demo_full.md) `[SRC]`, [JSON Lines](https://jsonlines.org/) và [3.08. Experiment and Benchmark Guide](../3.implementation/08.experiment_benchmark_guide.md) `[SRC]` — Lấy: event JSONL, temporal confirmation lite, quality gate và benchmark protocol. Dùng ở: minimal pipeline, source-of-truth log và block nên thêm ngay.
...
> **Nguồn tại mục:** [2.10. SOTIF for TSR](10.sotif_for_tsr.md) `[SRC]`, [2.09. ODD for TSR](09.odd_for_tsr.md) `[SRC]` và [3.12. Baseline Repo Analysis Full](../3.implementation/12.baseline_repo_analysis_full.md) `[SRC]` — Lấy: trigger conditions, ODD gap và baseline failure modes. Dùng ở: case demo chưa cover cho perception/system/HMI.
...
> **Nguồn tại mục:** [1.01. Prototype to Production Narrative](../1.narrative/01.prototype_to_production.md) `[SRC]`, [2.08. Verification Validation and Release](11.verification_validation_and_release.md) `[SRC]`, [3.10. Detector Architecture Deep Reference](../3.implementation/10.detector_architecture_deep_reference.md) `[SRC]` và [Delegated Regulation (EU) 2021/1958](https://eur-lex.europa.eu/eli/reg_del/2021/1958/oj) — Lấy: maturity ladder, release evidence, edge/deployment blocker và ISA authority context. Dùng ở: roadmap L0–L5, STATE-A–D mapping và Gantt.
...
> **Nguồn tại mục:** [3.12. Baseline Repo Analysis Full](../3.implementation/12.baseline_repo_analysis_full.md) `[SRC]`, [3.11. Colab Production-Lite Demo Full](../3.implementation/11.colab_production_lite_demo_full.md) `[SRC]` và [3.08. Experiment and Benchmark Guide](../3.implementation/08.experiment_benchmark_guide.md) `[SRC]` — Lấy: gap theo code, production-lite block và benchmark/replay sequencing. Dùng ở: thứ tự triển khai JSONL → lifecycle → diagnostics → parity/stub.
...
| Gap chi tiết theo code: [3.12. Baseline Repo Analysis Full](../3.implementation/12.baseline_repo_analysis_full.md) | Ma trận 14 gap → triệu chứng cụ thể |
...
> **Nguồn tại mục:** [2.06. Sources and Provenance](06.sources_and_provenance.md) `[SRC]` và các section chính trong file này — Lấy: mapping topic/source/location đã chuẩn hóa. Dùng ở: traceability matrix để tìm nơi dùng từng chủ đề production.

| Yêu cầu | Vị trí trong tài liệu này |
|---|---|
| Small Object Detection theory | [3.10. Detector Architecture Deep Reference](../3.implementation/10.detector_architecture_deep_reference.md) |
...
| Transformer-based TSR architectures | [3.10. Detector Architecture Deep Reference](../3.implementation/10.detector_architecture_deep_reference.md) |
| Dataset comparison | [3.10. Detector Architecture Deep Reference](../3.implementation/10.detector_architecture_deep_reference.md) §13 |
| KPI and validation metrics used by OEMs | II.10 (tổng quan), [3.12. Baseline Repo Analysis Full](../3.implementation/12.baseline_repo_analysis_full.md) §10–§12 |
| Failure Mode and Root Cause Analysis | I.9 (malfunction), [3.12. Baseline Repo Analysis Full](../3.implementation/12.baseline_repo_analysis_full.md) §13–§14 |
| SOTIF trigger conditions specific to TSR | II.17 |
| Edge deployment | [3.10. Detector Architecture Deep Reference](../3.implementation/10.detector_architecture_deep_reference.md) §14 |
...
> **Nguồn tại mục:** [3.10. Detector Architecture Deep Reference](../3.implementation/10.detector_architecture_deep_reference.md) `[SRC]`, [3.12. Baseline Repo Analysis Full](../3.implementation/12.baseline_repo_analysis_full.md) `[SRC]` và [2.06. Sources and Provenance](06.sources_and_provenance.md) `[SRC]` — Lấy: feedback review, dataset/source caveat và KPI/release evidence. Dùng ở: summary cách tài liệu đã xử lý phản biện.
...
|---|---|---|
| **Lead Domain Expert** | Cần công thức small-object và time-to-sign, không chỉ mô tả | [3.10. Detector Architecture Deep Reference](../3.implementation/10.detector_architecture_deep_reference.md) §12.1.1: $h_{net}$, $T_{ttf}$, stride, IoU sensitivity |
| **Rigorous Critic** | Dataset numbers phải có nguồn; không claim production từ VN HF | [3.10. Detector Architecture Deep Reference](../3.implementation/10.detector_architecture_deep_reference.md) §13.2 gắn URL chính thức; quadrant chart phân biệt research vs production |
| **Methodologist** | KPI phải có release gate, không chỉ liệt kê metric | [3.12. Baseline Repo Analysis Full](../3.implementation/12.baseline_repo_analysis_full.md) §10–§11: gate flow và SLIF ≥90% |
...
- Ngưỡng KPI "gợi ý" trong [3.12. Baseline Repo Analysis Full](../3.implementation/12.baseline_repo_analysis_full.md) §10.1 là pattern OEM thường dùng, **không** phải số contract cố định — mỗi OEM negotiate riêng.
...
> **Nguồn tại mục:** [2.06. Sources and Provenance](06.sources_and_provenance.md) `[SRC]` — Lấy: danh sách evidence nội bộ và vai trò từng file. Dùng ở: liên kết điều hướng cuối tài liệu.

- **(1) Tài liệu production mở rộng (file này):** [2.04. Unified Production Reference](04.unified_production_reference.md)
...
- **(2) Baseline gắn code:** [3.12. Baseline Repo Analysis Full](../3.implementation/12.baseline_repo_analysis_full.md)
- **(3) Nghiên cứu kiến trúc detector:** [3.10. Detector Architecture Deep Reference](../3.implementation/10.detector_architecture_deep_reference.md)
- Nguồn: [2.06. Sources and Provenance](06.sources_and_provenance.md)
