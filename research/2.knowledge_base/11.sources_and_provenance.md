...
- `[SRC]` Brief đầu bài: [0.requirements.md](../0.requirements.md) — Lấy: yêu cầu đề bài và scope học phần. Dùng ở: narrative, production roadmap và giới hạn claim của repo.
- `[SRC]` Narrative chính: [Prototype to Production Narrative](../1.narrative/01.prototype_to_production.md) — Lấy: mạch prototype → production và các gap cấp hệ thống. Dùng ở: entrypoint của các bài knowledge base.
- `[SRC]` Tài liệu production mở rộng: [Unified Production Reference](12.unified_production_reference.md) — Lấy: production stack, safety/SOTIF/HMI/diagnostics/release roadmap. Dùng ở: source chuẩn nội bộ cho kiến trúc feature TSR.
- `[SRC]` Baseline gắn code: [Baseline Repo Analysis Full](../3.implementation/04.baseline_repo_analysis_full.md) — Lấy: phân tích chi tiết code, runtime, checkpoint và gap. Dùng ở: các mục “Relation to Current Prototype”.
- `[SRC]` Nghiên cứu kiến trúc detector + edge deployment: [Detector Architecture Deep Reference](13.detector_architecture_deep_reference.md) — Lấy: detector taxonomy, neck/head/NMS, edge runtime và dataset benchmark. Dùng ở: detector overview, dataset guide và architecture deep dive.
- `[SRC]` Notebook production-lite: [Colab Production-Lite Demo Full](../3.implementation/05.colab_production_lite_demo_full.md) — Lấy: quality gate, ODD gate, lifecycle state, event JSONL và KPI proxy. Dùng ở: state manager, diagnostics-lite, V&V và ODD examples.
- `[SRC]` Visual appendix: [System Diagram](10.system_diagram.md) — Lấy: block diagram và signal flow camera → detector → state/context → HMI/diagnostics. Dùng ở: architecture overview và slide/narrative visuals.
- `[SRC]` Slide deck chi tiết: [Slide Deck Extended](../4.presentation/03.slide_deck_full_reference.md) — Lấy: cấu trúc trình bày cho hội đồng. Dùng ở: presentation traceability và executive summary.
- `[SRC]` Notebook tái lập nhẹ: [06.tsr_research_workflow.ipynb](../3.implementation/notebooks/06.tsr_research_workflow.ipynb) — Lấy: workflow tái lập nghiên cứu nhẹ. Dùng ở: provenance local replay và benchmark discipline.
- `[SRC]` Notebook Colab production-lite: [07.tsr_colab_production_lite_demo.ipynb](../3.implementation/notebooks/07.tsr_colab_production_lite_demo.ipynb) — Lấy: demo notebook runnable cho production-lite concepts. Dùng ở: evidence notebook và so sánh với bản markdown full.
...
- `[SRC]` Notebook `research/3.implementation/notebooks/07.tsr_colab_production_lite_demo.ipynb`, `README.md`, và dấu vết cục bộ trong checkpoint `models/best.pt` như `yolo11s.yaml`, `yolo11s.pt`, `traffic_sign_yolo11s`, `data.yaml` được dùng như local corroboration cho provenance của model.
...
- Các nguồn này được dùng để củng cố phần ODD, driving modes, hệ thống state transition, và verification logic trong [Unified Production Reference](12.unified_production_reference.md) `[SRC]`.
...
- Các nguồn trong mục này hỗ trợ [Detector Architecture Deep Reference](13.detector_architecture_deep_reference.md) `[SRC]` cho small-object, edge, dataset và [Unified Production Reference](12.unified_production_reference.md) `[SRC]` cho ISA, SOTIF, fusion.
...
- Số liệu dataset trong [Detector Architecture Deep Reference](13.detector_architecture_deep_reference.md) `[SRC]` §13.2 được đối chiếu với các trang chính thức trên.
...
- Council Review Summary nằm ở [Unified Production Reference](12.unified_production_reference.md) `[SRC]` Phụ lục C.
