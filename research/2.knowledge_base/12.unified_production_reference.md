...
| Thuộc tính | Giá trị |
|---|---|
| Narrative chính | [Prototype to Production Narrative](../1.narrative/01.prototype_to_production.md) `[SRC]` |
| Brief đầu bài | [0.requirements.md](../0.requirements.md) `[SRC]` |
...
| Tài liệu chuyên đề | [Baseline Repo Analysis Full](../3.implementation/04.baseline_repo_analysis_full.md) `[SRC]`, [Detector Architecture Deep Reference](13.detector_architecture_deep_reference.md) `[SRC]`, [Colab Production-Lite Demo Full](../3.implementation/05.colab_production_lite_demo_full.md) `[SRC]` |
| Tài liệu dẫn xuất | [System Diagram](10.system_diagram.md) `[SRC]`, [Slide Deck Extended](../4.presentation/03.slide_deck_full_reference.md) `[SRC]` |
| Sơ đồ HTML Phần II | [assets/diagrams/tsr_production_ecu_architecture.html](../assets/diagrams/tsr_production_ecu_architecture.html) `[SRC]` |
| Nguồn tham khảo | [Sources and Provenance](11.sources_and_provenance.md) `[SRC]` |
...
| Folder | Index |
|---|---|
| Knowledge Base | [00.index.md](00.index.md) |
| Implementation | [../3.implementation/00.index.md](../3.implementation/00.index.md) |
| Presentation | [../4.presentation/00.index.md](../4.presentation/00.index.md) |

File này chỉ giữ phần production deep reference.

### 0.2 Nguồn chuẩn và link nhanh

Các phần safety, SOTIF, HMI, diagnostics, cybersecurity, OTA và ISA trong tài liệu này dùng các nguồn chính dưới đây. Khi viết báo cáo hoặc slide, ưu tiên link này và đối chiếu thêm [Sources and Provenance](11.sources_and_provenance.md) `[SRC]`.
...
> **Nguồn tại mục:** [Unified Production Reference](12.unified_production_reference.md) `[SRC]`, [State Manager and Sign Lifecycle](02.state_manager_and_sign_lifecycle.md) `[SRC]` và [ISO 21448:2022](https://www.iso.org/standard/77490.html) — Lấy: phân lớp perception → temporal/context → feature output và ODD/SOTIF gate. Dùng ở: bảng năm lớp xử lý trước khi publish HMI/ISA.
...
> **Nguồn tại mục:** [ISO 26262 package overview](https://www.iso.org/publication/PUB200262.html), [ISO 21448:2022](https://www.iso.org/standard/77490.html) và [ODD for TSR](03.odd_for_tsr.md) `[SRC]` — Lấy: item definition, authority, ODD và fallback/degraded behavior. Dùng ở: item definition phase-1 cho TSR advisory-only.
...
> **Nguồn tại mục:** [ISO 26262-1:2018](https://www.iso.org/standard/68383.html), [Automotive SPICE / VDA QMC](https://vda-qmc.de/en/automotive-spice/) và [Verification Validation and Release](05.verification_validation_and_release.md) `[SRC]` — Lấy: lifecycle evidence, traceability và release gate mindset. Dùng ở: chuỗi work products item definition → verification → release readiness.
...
> **Nguồn tại mục:** [System Diagram](10.system_diagram.md) `[SRC]`, [AUTOSAR Classic Platform](https://www.autosar.org/standards/classic-platform) và [ISO 14229-1:2026](https://www.iso.org/standard/87962.html) — Lấy: block decomposition, ECU/software boundary và diagnostics interface. Dùng ở: kiến trúc camera/ECU/perception/state/HMI/gateway/service.
...
> **Nguồn tại mục:** [ISO 16750-1:2023](https://www.iso.org/standard/77578.html), [AUTOSAR Classic Platform](https://www.autosar.org/standards/classic-platform) và [Detector Architecture Deep Reference](13.detector_architecture_deep_reference.md) `[SRC]` — Lấy: vehicle environmental robustness, platform boundary và edge/runtime constraints. Dùng ở: camera, ECU, storage, network, power, thermal và hardware trade-off của TSR.
...
> **Nguồn tại mục:** [AUTOSAR Classic Platform](https://www.autosar.org/standards/classic-platform), [ISO 26262-1:2018](https://www.iso.org/standard/68383.html) và [Baseline Repo Analysis Full](../3.implementation/04.baseline_repo_analysis_full.md) `[SRC]` — Lấy: phân lớp SWC/platform, fault containment và gap monolithic script. Dùng ở: task-based architecture, health monitor, degraded operation và timing contract.
...
> **Nguồn tại mục:** [ISO 21448:2022](https://www.iso.org/standard/77490.html), [Setting AI in context](https://arxiv.org/abs/2201.11451) và [ODD for TSR](03.odd_for_tsr.md) `[SRC]` — Lấy: ODD boundary, trigger conditions và state/degraded behavior. Dùng ở: phase-1 ODD, out-of-scope ODD và state flow `INIT` → `SAFE_DISABLE`.
...
> **Nguồn tại mục:** [Glossary](09.glossary.md) `[SRC]` và [Sources and Provenance](11.sources_and_provenance.md) `[SRC]` — Lấy: thuật ngữ chuẩn nội bộ và provenance của từng nhóm nguồn. Dùng ở: bảng thuật ngữ nền, common mistakes và cách đọc khái niệm trong toàn bộ research.
...
> **Nguồn tại mục:** [Baseline Repo Analysis Full](../3.implementation/04.baseline_repo_analysis_full.md) `[SRC]`, [Prototype to Production Narrative](../1.narrative/01.prototype_to_production.md) `[SRC]` và [System Diagram](10.system_diagram.md) `[SRC]` — Lấy: gap detector frame-level, block production cần thêm và kiến trúc feature. Dùng ở: sáu gap baseline và stack `Perception/Feature/Vehicle Integration`.
...
> **Nguồn tại mục:** [System Diagram](10.system_diagram.md) `[SRC]`, [State Manager and Sign Lifecycle](02.state_manager_and_sign_lifecycle.md) `[SRC]`, [HMI Diagnostics and Vehicle Interface](06.hmi_diagnostics_and_vehicle_interface.md) `[SRC]` và [AUTOSAR Classic Platform](https://www.autosar.org/standards/classic-platform) — Lấy: pipeline camera → quality gate → detector → state/context → HMI/diagnostics và ECU layering. Dùng ở: sơ đồ end-to-end và phân rã block theo trách nhiệm.
...
> **Nguồn tại mục:** [ISO 26262-1:2018](https://www.iso.org/standard/68383.html), [ISO 21448:2022](https://www.iso.org/standard/77490.html), [UNECE R156](https://unece.org/transport/documents/2021/03/standards/un-regulation-no-156-software-update-and-software-update) và [Verification Validation and Release](05.verification_validation_and_release.md) `[SRC]` — Lấy: lifecycle gate, ODD/trigger validation và update/rollback evidence. Dùng ở: bảng concept → release của TSR.
...
> **Nguồn tại mục:** [SORT](https://arxiv.org/abs/1602.00763), [Deep SORT](https://arxiv.org/abs/1703.07402), [State Manager and Sign Lifecycle](02.state_manager_and_sign_lifecycle.md) `[SRC]` và [Colab Production-Lite Demo Full](../3.implementation/05.colab_production_lite_demo_full.md) `[SRC]` — Lấy: tracking-by-detection, association qua miss/occlusion, lifecycle state và hit/miss counters. Dùng ở: tracking policy, sign lifecycle và gap `hold` của repo.
...
> **Nguồn tại mục:** [ISO 21448:2022](https://www.iso.org/standard/77490.html), [MapFusion](https://arxiv.org/abs/2103.05929), [ODD for TSR](03.odd_for_tsr.md) `[SRC]` và [HMI Diagnostics and Vehicle Interface](06.hmi_diagnostics_and_vehicle_interface.md) `[SRC]` — Lấy: ODD/context gate, camera-map fusion concept và publish policy. Dùng ở: ba câu hỏi lane/context/fusion trước khi publish sign.
...
> **Nguồn tại mục:** [ISO 26262-1:2018](https://www.iso.org/standard/68383.html), [ISO 21448:2022](https://www.iso.org/standard/77490.html), [Automotive SPICE / VDA QMC](https://vda-qmc.de/en/automotive-spice/) và [Baseline Repo Analysis Full](../3.implementation/04.baseline_repo_analysis_full.md) `[SRC]` — Lấy: verification lifecycle, scenario validation, process evidence và KPI/release gates. Dùng ở: V-model, test gates, KPI production và embedded release checklist.

> **KPI gates, release flow và failure RCA chi tiết:** [Baseline Repo Analysis Full](../3.implementation/04.baseline_repo_analysis_full.md) — Lấy: evidence implementation cho KPI gates, release flow và failure RCA; xem §10–§14. Dùng ở: phần release/checklist của production reference, không lặp bảng KPI gate §10.1 hoặc release flow §11.
...
> **Nguồn tại mục:** [ISO 26262-1:2018](https://www.iso.org/standard/68383.html), [ISO 21448:2022](https://www.iso.org/standard/77490.html), [UNECE R156](https://unece.org/transport/documents/2021/03/standards/un-regulation-no-156-software-update-and-software-update) và [Verification Validation and Release](05.verification_validation_and_release.md) `[SRC]` — Lấy: evidence package, validation work products và update/rollback concept. Dùng ở: checklist trước pilot.
...
> **Nguồn tại mục:** [Requirements Engineering for Automotive Perception Systems](https://arxiv.org/abs/2302.12155), [ODD for TSR](03.odd_for_tsr.md) `[SRC]` và [System Diagram](10.system_diagram.md) `[SRC]` — Lấy: perception requirements theo context/operating condition và lane/context block trong TSR. Dùng ở: ego-lane applicability, topology pending và camera-only stub boundary.
...
> **Nguồn tại mục:** [ISO 21448:2022](https://www.iso.org/standard/77490.html), [ODD for TSR](03.odd_for_tsr.md) `[SRC]` và [SOTIF for TSR](04.sotif_for_tsr.md) `[SRC]` — Lấy: trigger condition, ODD/quality gate và residual-risk mitigation. Dùng ở: reason-coded suppression, applicability confidence và degraded/unavailable behavior.
...
| ETSC ISA overview | https://etsc.eu/intelligent-speed-assistance-isa/ | Public explainer về ISA và adoption context. | Bối cảnh presentation/narrative, không thay thế regulation chính thức. |
...
> **Nguồn tại mục:** [ISO 21448:2022](https://www.iso.org/standard/77490.html), [Organization of ML-based product development as per ISO 26262 and ISO/PAS 21448](https://arxiv.org/abs/1910.05112), [GLARE Traffic Sign Dataset](https://arxiv.org/abs/2209.08716) và [SOTIF for TSR](04.sotif_for_tsr.md) `[SRC]` — Lấy: SOTIF trigger condition, ML evidence lifecycle và glare dataset example. Dùng ở: trigger catalog, mitigation pattern và residual risk argument.
...
> **Nguồn tại mục:** [ISO 26262-1:2018](https://www.iso.org/standard/68383.html), [ISO 21448:2022](https://www.iso.org/standard/77490.html), [Automotive SPICE / VDA QMC](https://vda-qmc.de/en/automotive-spice/) và [Verification Validation and Release](05.verification_validation_and_release.md) `[SRC]` — Lấy: verification lifecycle, scenario validation và release process evidence. Dùng ở: SIL/MIL/HIL/vehicle/shadow/fleet gate chain.
...
> **Nguồn tại mục:** [ISO 26262-1:2018](https://www.iso.org/standard/68383.html), [ISO 21448:2022](https://www.iso.org/standard/77490.html), [UNECE R156](https://unece.org/transport/documents/2021/03/standards/un-regulation-no-156-software-update-and-software-update) và [Verification Validation and Release](05.verification_validation_and_release.md) `[SRC]` — Lấy: safety lifecycle evidence, SOTIF residual risk và post-release/update governance. Dùng ở: safety case package, authority-level evidence và residual risk review.
...
> **Nguồn tại mục:** [Baseline Repo Analysis Full](../3.implementation/04.baseline_repo_analysis_full.md) `[SRC]`, [`code/tsr_demo.py`](../../code/tsr_demo.py) `[SRC]` và [Prototype to Production Narrative](../1.narrative/01.prototype_to_production.md) `[SRC]` — Lấy: trạng thái baseline runtime/code và gap production. Dùng ở: phân loại repo là research/prototype, chưa production-ready.
...
> **Nguồn tại mục:** [Baseline Repo Analysis Full](../3.implementation/04.baseline_repo_analysis_full.md) `[SRC]`, [Colab Production-Lite Demo](../3.implementation/02.colab_production_lite_demo.md) `[SRC]` và [Notebook Analyze](../3.implementation/10.notebook_analyze.md) `[SRC]` — Lấy: số đo baseline lịch sử, video replay hiện có `59.94 fps`, và lịch `skip` của notebook `07`. Dùng ở: tách rõ baseline benchmark, video FPS, scheduled inference rate và measured processing FPS.

Hiện tại notebook `07` dùng `skip=0` cho profile `high/medium` và `skip=1` cho profile `low`. Video replay hiện có là `videos/14212806_3840_2160_60fps.mp4` với `3840x2160`, `1187` frame, `59.94 fps`.

---

## III.3 Runtime budget theo state (RAM thấp)

> **Nguồn tại mục:** [Baseline Repo Analysis Full](../3.implementation/04.baseline_repo_analysis_full.md) `[SRC]`, [Ultralytics Benchmark Mode](https://docs.ultralytics.com/modes/benchmark/) và [Detector Architecture Deep Reference](13.detector_architecture_deep_reference.md) `[SRC]` — Lấy: runtime local, benchmark discipline và edge-runtime constraints. Dùng ở: RAM/FPS/latency budget theo state và degraded mode.
...
> **Nguồn tại mục:** [Colab Production-Lite Demo Full](../3.implementation/05.colab_production_lite_demo_full.md) `[SRC]`, [JSON Lines](https://jsonlines.org/) và [Experiment and Benchmark Guide](../3.implementation/03.experiment_benchmark_guide.md) `[SRC]` — Lấy: event JSONL, temporal confirmation lite, quality gate và benchmark protocol. Dùng ở: minimal pipeline, source-of-truth log và block nên thêm ngay.
...
> **Nguồn tại mục:** [SOTIF for TSR](04.sotif_for_tsr.md) `[SRC]`, [ODD for TSR](03.odd_for_tsr.md) `[SRC]` và [Baseline Repo Analysis Full](../3.implementation/04.baseline_repo_analysis_full.md) `[SRC]` — Lấy: trigger conditions, ODD gap và baseline failure modes. Dùng ở: case demo chưa cover cho perception/system/HMI.
...
> **Nguồn tại mục:** [Prototype to Production Narrative](../1.narrative/01.prototype_to_production.md) `[SRC]`, [Verification Validation and Release](05.verification_validation_and_release.md) `[SRC]`, [Detector Architecture Deep Reference](13.detector_architecture_deep_reference.md) `[SRC]` và [Delegated Regulation (EU) 2021/1958](https://eur-lex.europa.eu/eli/reg_del/2021/1958/oj) — Lấy: maturity ladder, release evidence, edge/deployment blocker và ISA authority context. Dùng ở: roadmap L0–L5, STATE-A–D mapping và Gantt.
...
> **Nguồn tại mục:** [Baseline Repo Analysis Full](../3.implementation/04.baseline_repo_analysis_full.md) `[SRC]`, [Colab Production-Lite Demo Full](../3.implementation/05.colab_production_lite_demo_full.md) `[SRC]` và [Experiment and Benchmark Guide](../3.implementation/03.experiment_benchmark_guide.md) `[SRC]` — Lấy: gap theo code, production-lite block và benchmark/replay sequencing. Dùng ở: thứ tự triển khai JSONL → lifecycle → diagnostics → parity/stub.
...
| Gap chi tiết theo code: [Baseline Repo Analysis Full](../3.implementation/04.baseline_repo_analysis_full.md) | Ma trận 14 gap → triệu chứng cụ thể |
...
> **Nguồn tại mục:** [Sources and Provenance](11.sources_and_provenance.md) `[SRC]` và các section chính trong file này — Lấy: mapping topic/source/location đã chuẩn hóa. Dùng ở: traceability matrix để tìm nơi dùng từng chủ đề production.

| Yêu cầu | Vị trí trong tài liệu này |
|---|---|
| Small Object Detection theory | [Detector Architecture Deep Reference](13.detector_architecture_deep_reference.md) |
...
| Transformer-based TSR architectures | [Detector Architecture Deep Reference](13.detector_architecture_deep_reference.md) |
| Dataset comparison | [Detector Architecture Deep Reference](13.detector_architecture_deep_reference.md) §13 |
| KPI and validation metrics used by OEMs | II.10 (tổng quan), [Baseline Repo Analysis Full](../3.implementation/04.baseline_repo_analysis_full.md) §10–§12 |
| Failure Mode and Root Cause Analysis | I.9 (malfunction), [Baseline Repo Analysis Full](../3.implementation/04.baseline_repo_analysis_full.md) §13–§14 |
| SOTIF trigger conditions specific to TSR | II.17 |
| Edge deployment | [Detector Architecture Deep Reference](13.detector_architecture_deep_reference.md) §14 |
...
> **Nguồn tại mục:** [Detector Architecture Deep Reference](13.detector_architecture_deep_reference.md) `[SRC]`, [Baseline Repo Analysis Full](../3.implementation/04.baseline_repo_analysis_full.md) `[SRC]` và [Sources and Provenance](11.sources_and_provenance.md) `[SRC]` — Lấy: feedback review, dataset/source caveat và KPI/release evidence. Dùng ở: summary cách tài liệu đã xử lý phản biện.
...
|---|---|---|
| **Lead Domain Expert** | Cần công thức small-object và time-to-sign, không chỉ mô tả | [Detector Architecture Deep Reference](13.detector_architecture_deep_reference.md) §12.1.1: $h_{net}$, $T_{ttf}$, stride, IoU sensitivity |
| **Rigorous Critic** | Dataset numbers phải có nguồn; không claim production từ VN HF | [Detector Architecture Deep Reference](13.detector_architecture_deep_reference.md) §13.2 gắn URL chính thức; quadrant chart phân biệt research vs production |
| **Methodologist** | KPI phải có release gate, không chỉ liệt kê metric | [Baseline Repo Analysis Full](../3.implementation/04.baseline_repo_analysis_full.md) §10–§11: gate flow và SLIF ≥90% |
...
- Ngưỡng KPI "gợi ý" trong [Baseline Repo Analysis Full](../3.implementation/04.baseline_repo_analysis_full.md) §10.1 là pattern OEM thường dùng, **không** phải số contract cố định — mỗi OEM negotiate riêng.
...
> **Nguồn tại mục:** [Sources and Provenance](11.sources_and_provenance.md) `[SRC]` — Lấy: danh sách evidence nội bộ và vai trò từng file. Dùng ở: liên kết điều hướng cuối tài liệu.

- **(1) Tài liệu production mở rộng (file này):** [Unified Production Reference](12.unified_production_reference.md)
- Sơ đồ ECU HTML: [assets/diagrams/tsr_production_ecu_architecture.html](../assets/diagrams/tsr_production_ecu_architecture.html)
- **(2) Baseline gắn code:** [Baseline Repo Analysis Full](../3.implementation/04.baseline_repo_analysis_full.md)
- **(3) Nghiên cứu kiến trúc detector:** [Detector Architecture Deep Reference](13.detector_architecture_deep_reference.md)
- Nguồn: [Sources and Provenance](11.sources_and_provenance.md)
