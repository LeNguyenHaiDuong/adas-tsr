...
> **Thứ tự đọc:** 3 — appendix provenance, không phải narrative chính.
>
...
- `[SRC]` Tài liệu production mở rộng: [2.01. Unified Production Reference](01.unified_production_reference.md) — Lấy: production stack, safety/SOTIF/HMI/diagnostics/release roadmap. Dùng ở: source chuẩn nội bộ cho kiến trúc feature TSR.
- `[SRC]` ISP/image pipeline research: [2.10. ISP Image Pipeline for TSR](10.isp_image_pipeline_for_tsr.md) — Lấy: CMOS/RAW Bayer/ISP pipeline, AE, AWB, demosaicing, denoising, HDR, tone mapping, lens correction, sharpening và ảnh hưởng tới YOLO/TSR. Dùng ở: giải thích input quality trước detector và phân biệt ISP thật với software preprocessing trong notebook.
- `[SRC]` Baseline gắn code: [3.08. Baseline Repo Analysis Full](../3.implementation/08.baseline_repo_analysis_full.md) — Lấy: phân tích chi tiết code, runtime, checkpoint và gap. Dùng ở: các mục “Relation to Current Prototype”.
- `[SRC]` Nghiên cứu kiến trúc detector + edge deployment: [3.06. Detector Architecture Deep Reference](../3.implementation/06.detector_architecture_deep_reference.md) — Lấy: detector taxonomy, neck/head/NMS, edge runtime và dataset benchmark. Dùng ở: detector overview, dataset guide và architecture deep dive.
- `[SRC]` Notebook production-lite: [3.07. Colab Production-Lite Demo Full](../3.implementation/07.colab_production_lite_demo_full.md) — Lấy: quality gate, ODD gate, lifecycle state, event JSONL và KPI proxy. Dùng ở: state manager, diagnostics-lite, V&V và ODD examples.
- `[SRC]` Visual appendix: [2.02. System Diagram](02.system_diagram.md) — Lấy: block diagram và signal flow camera → detector → state/context → HMI/diagnostics. Dùng ở: architecture overview và slide/narrative visuals.
- Slide deck cũ trong `research/4.presentation/` đã được loại khỏi repo theo phạm vi tài liệu tuyến tính hiện tại; các claim presentation nên trace về Narrative, Knowledge Base và Implementation index.
...
- Các nguồn này được dùng để củng cố phần ODD, driving modes, hệ thống state transition, và verification logic trong [2.01. Unified Production Reference](01.unified_production_reference.md) `[SRC]`.
...
- [European Transport Safety Council — Intelligent Speed Assistance overview](https://etsc.eu/intelligent-speed-assistance-isa/) — Lấy: public explainer về ISA deployment/safety context. Dùng ở: presentation/narrative bối cảnh, không thay thế regulation chính thức.
...
- [OpenCV color conversions and demosaicing](https://docs.opencv.org/4.x/d8/d01/group__imgproc__color__conversions.html) — Lấy: Bayer demosaicing API và các mode interpolation. Dùng ở: [2.10. ISP Image Pipeline for TSR](10.isp_image_pipeline_for_tsr.md).
- [OpenCV xphoto white balance and tonemap](https://docs.opencv.org/5.0/extra_modules/xphoto.html) — Lấy: Gray-world/simple/learning-based white balance và tone mapping reference. Dùng ở: [2.10. ISP Image Pipeline for TSR](10.isp_image_pipeline_for_tsr.md).
- [OpenCV denoising](https://docs.opencv.org/4.x/d1/d79/group__photo__denoise.html) — Lấy: Non-local Means denoising và tham số filter strength/window. Dùng ở: [2.10. ISP Image Pipeline for TSR](10.isp_image_pipeline_for_tsr.md).
- [OpenCV HDR imaging](https://docs.opencv.org/4.x/d6/df5/group__photo__hdr.html) — Lấy: merge exposure, exposure fusion và tonemapping. Dùng ở: [2.10. ISP Image Pipeline for TSR](10.isp_image_pipeline_for_tsr.md).
- [OpenCV camera calibration](https://docs.opencv.org/4.x/dc/dbb/tutorial_py_calibration.html) — Lấy: intrinsic/extrinsic calibration, undistortion và remap. Dùng ở: [2.10. ISP Image Pipeline for TSR](10.isp_image_pipeline_for_tsr.md).
- [NVIDIA VPI Lens Distortion Correction](https://docs.nvidia.com/vpi/html/algo_ldc.html) — Lấy: lens distortion correction/remap trong embedded vision pipeline. Dùng ở: [2.10. ISP Image Pipeline for TSR](10.isp_image_pipeline_for_tsr.md).
...
- Các nguồn trong mục này hỗ trợ [3.06. Detector Architecture Deep Reference](../3.implementation/06.detector_architecture_deep_reference.md) `[SRC]` cho small-object, edge, dataset và [2.01. Unified Production Reference](01.unified_production_reference.md) `[SRC]` cho ISA, SOTIF, fusion.
...
- [InterRegs ISA timeline analysis](https://www.interregs.com/articles/spotlight/236/eu-regulation-on-intelligent-speed-assistance-systems-published-) — Lấy: public timeline summary for EU ISA dates. Dùng ở: presentation/narrative timeline only; compliance text vẫn cần đối chiếu EU regulation.

Ghi chú:

- Số liệu dataset trong [3.06. Detector Architecture Deep Reference](../3.implementation/06.detector_architecture_deep_reference.md) `[SRC]` §13.2 được đối chiếu với các trang chính thức trên.
...
- Council Review Summary nằm ở [2.01. Unified Production Reference](01.unified_production_reference.md) `[SRC]` Phụ lục C.
