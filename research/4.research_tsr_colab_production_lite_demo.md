...
## 2. Notebook này bám vào tài liệu nào?

### 2.1 Bám theo tài liệu thống nhất

Notebook lấy ý tưởng trực tiếp từ các phần sau trong [1.research_tsr_three_part_unified.md](1.research_tsr_three_part_unified.md):

- **I.3** — item definition phase-1: `advisory only`, ODD hẹp, có `DEGRADED` / `UNAVAILABLE`.
- **I.7.5** — `fault containment` và `degraded operation`.
- **I.8** — `ODD phase-1` và `state flow`.
- **II.1.2** — detector không phải feature.
- **II.5.3** — `sign lifecycle` và `state machine`.
- **II.9** — `HMI policy production`.
- **II.14** — `context filtering` và `reason-coded suppression`.

### 2.2 Bám theo phân tích baseline

Notebook cũng phản ánh các gap đã nêu trong [2.research_tsr_baseline_analysis.md](2.research_tsr_baseline_analysis.md):

- baseline chỉ overlay, chưa có output machine-readable;
- `hold` không phải tracking thật;
- chưa có quality gate;
- chưa có stateful feature logic;
- chưa có event log để replay nguyên nhân publish hoặc suppress.

### 2.3 Bám theo nghiên cứu detector

Phần detector trong notebook không cố thay model. Nó tiếp tục dùng YOLO baseline và chấp nhận các giới hạn small-object, latency và long-tail đã phân tích trong [3.research_tsr_detection_architecture_research.md](3.research_tsr_detection_architecture_research.md).

### 2.4 Legend màu Mermaid
...
flowchart LR
    A[Input Video]
    --> B[RAM Profile Selection]
    --> C[Image Quality Gate]
    --> D[ODD Gate]
    --> E[YOLO Detector]
    --> F[Tracking and Temporal Confirmation]
    --> G[Sign Lifecycle]
    --> H[Warning Level and HMI Policy]
    --> I[Annotated Output Video]
